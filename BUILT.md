...
- JWT + admin/operator/viewer (read): agents/policies/tools/nodes list+get, `/v1/audit`, `/v1/audit/export`, `/v1/audit/verify`, `/v1/alerts*`, `/v1/finops/summary`, `/v1/finops/timeseries`, `/v1/memory/episodes`, `/v1/memory/episodes/search`, `/v1/gateway/episodes`, `/v1/gateway/episodes/search`, `/v1/gateway/episodes/{episodeId}` (B-002 Brief 2, see below), `/v1/endpoints*`, `/v1/discover/endpoints*`
...
**Interfaces — B-002 Brief 2 (2026-07-22, branch `b-002-eami-api-proxy-layer`), gateway episode proxy:** new file `internal/api/gateway_episodes.go`. `GET /v1/gateway/episodes` (+`/search`, `/{episodeId}`) proxy to `eami-gateway`'s Brief-1 endpoint via `X-Service-Key` (config `Gateway.EpisodeReadServiceKey`, env `GATEWAY_EPISODE_READ_SERVICE_KEY` — same var name/value eami-gateway itself validates inbound; `Gateway.URL`/env `API_GATEWAY_URL`, defaults `http://eami-gateway:8080`). Both config fields optional at eami-api startup — unset means the three routes 502 per-request ("gateway proxy is not configured"), not a boot failure. **Purely additive: `/v1/memory/episodes*` and `memory.go` are completely untouched (zero lines changed) — both route families coexist until Brief 3.** Org isolation (the actual point of this brief, since Brief 1's service-key path enforces none): every gateway call's `org_id` is always `claimsFromContext(r).OrgID`, never client input; an optional `org_id` query param is accepted only as a tamper-check that 403s on mismatch before the gateway is ever called. Gateway error responses collapse to a real 404 (episode not found / cross-org, same value) or a generic 502 (everything else, including network failures) — gateway's plain-text body is never read past the status code, so nothing can leak through. New exported types `GatewayEpisode`/`GatewayListResult`/`GatewayError`/`GatewayEpisodeClient` (exported only so the external `api_test` test package can construct fakes/errors — not intended as a public API). `router.go`'s `NewServer`/`NewHandler` gained a `gatewayClient` field and a `WithGatewayClient` test-injection method; fixed a nil-`cfg` panic in `NewServer` surfaced by `finops_test.go`'s existing `NewServer(nil,...)` call while wiring this in.
**Test coverage:** 6 `_test.go` files vs 44 source files. Covers agents, approvals, auth, finops, policies, and (new, Brief 2) the gateway episode proxy — 11 tests including the org-isolation centerpiece (`TestGatewayEpisodes_List_MismatchedOrgIDSupplied_Returns403_GatewayNeverCalled`, asserts both the 403 and that the fake gateway client's call count is zero). **Verified 2026-07-22 with a real toolchain: `go build ./...`, `go vet ./...`, `go test ./...` all clean, 0 failures.** Two subagent reviewer/security passes on the diff also came back clean (see completion report for B-002 Brief 2). **Pre-existing, unrelated issue discovered while running tests:** several `TestFinOpsTimeSeries_*` subtests in `finops_test.go` panic internally (nil `s.queries` dereferenced in `finops.go:269`) but still report `--- PASS` because chi's `Recoverer` catches the panic and the tests don't assert against that specific response; confirmed via `git diff --stat master -- finops.go finops_test.go` (zero changes on this branch) that this predates Brief 2 entirely. Not fixed here (out of scope) — see `BACKLOG.md` B-016.
**Known limitations / landmines:**
- **⚠️ `internal/api/memory.go` (`ListMemoryEpisodes`, `SearchMemoryEpisodes`) still returns full episode content — including the full `Steps` JSONB (tool calls, args, results) — to the SaaS API.** This is a direct conflict with **ADR-010** and was shipped despite **TASK-069/070's explicit instructions**. **ADR-019 itself is now Accepted/resolved (`DECISIONS.md`), and Brief 1 + Brief 2 of the 3-brief fix are done and merged/branched** — but `memory.go` is *unchanged* until Brief 3 lands (deliberately, per both briefs' scope). Until then this landmine is still live: `memory.go`'s routes still violate the now-accepted ADR, running in parallel with the new, compliant `/v1/gateway/episodes*` routes. See `BACKLOG.md` B-002.

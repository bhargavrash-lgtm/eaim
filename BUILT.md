...
**Test coverage:** 3 `_test.go` files vs 13 source files. Includes `TestManager_Validate_RevokedToken_SurvivesRestart`, `TestManager_Validate_WrongIssuer_ReturnsError`, `TestWriter_DBErrorOnInit_PropagatesError`. Not executed this session.
...
- **⚠️ ADR-010 / ADR-019 conflict (see `eami-api` section below) — episode full content now reaches the SaaS API, which ADR-010 says should never happen, via a path TASK-069 explicitly said not to build until ADR-019 resolves. ADR-019 is still listed "Pending" in `DECISIONS.md`.**

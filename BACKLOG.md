...
### B-028 — `eami-api.yaml`/`eami-gateway.yaml` still ship literal `"changeme"` default values
...
### B-030 — `scripts/seed-db.sh`/`scripts/create-audit-partition.sh` have the same silent-weak-default pattern B-025 closed elsewhere
...
### B-031 — `API_JWT_SECRET` is dead config that implies an HS256 fallback which doesn't exist
...
**Acceptance criteria:**
- [ ] Either remove `API_JWT_SECRET` entirely from `scripts/setup.sh`, `.env.example`, `docker-compose.yml`, and `docker-compose.prod.yml`, **or** keep it but rewrite the `.env.example` comment (and any inline compose/setup.sh comments) to say plainly that it's unused/reserved, not an active HS256 fallback
- [ ] If removing: confirm nothing else references `API_JWT_SECRET` (e.g. no test, no doc outside this cleanup, no downstream tooling) before deleting it from the generator/compose/env files
- [ ] If keeping as documented-but-reserved: make clear in the comment *why* it's kept (e.g. reserved for a possible future HS256 fallback) rather than leaving it ambiguous

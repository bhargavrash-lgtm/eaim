...
- **Toolchain note:** this dev machine has no `go` or `node`/`npm` on PATH. Backend/frontend build and test status in `BUILT.md` is from static source review, not executed runs — verify locally before trusting green.

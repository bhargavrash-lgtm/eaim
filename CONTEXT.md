...
Audit exports are intentionally bounded all-or-error: 10,000 rows, 20 MiB serialized CSV, 15-second query timeout, and 1 MiB per selected text field. A read-only repeatable-read transaction preflights count, field size, and a conservative CSV byte estimate before rows stream into the bounded buffer. Formula-safe CSV text, static filenames, one active export per org, and four starts per org/minute protect the data-egress path. Existing JWT/RBAC plus SQL org scoping remains in force; the FinOps team join now requires both agent and usage orgs to match. `audit_export_pg_test.go` proves filter and same-interval cross-org exclusion; `finops_pg_test.go` proves a foreign agent owner cannot leak and locks down the date boundary.

Validation complete: `go test ./internal/api ./internal/store`, `npx tsc --noEmit`, `npx vite build`, and Docker API rebuild/restart all pass; `/v1/audit/export` returns 401 with no JWT. Both mandatory reviewers passed after correcting bounded scan memory, concurrent-export admission, stale FinOps error data, cross-org fixture timing, and rate-limit ordering. The local UI is at sign-in and targeted real-Postgres tests skip without a supplied test credential; neither browser CSV download nor live cross-org HTTP test was claimed or performed, and no secret was read. B-222 is queued for Architect-EAMI to align the OpenAPI export contract. B-221 marker cleared at completion.

## Active decision thread (2026-09-25) — B-221 Part A investigation (historical)

PM's supplied brief explicitly requires an investigation report before an implementation plan or code. B-221 was confirmed free via a fresh BACKLOG scan and reserved; next ID is B-222. Marker was `none` at entry, acquired by Codex at 04:50:03Z, and released at this investigation handoff. Horizon-0-adjacent Audit/FinOps maturity work, MATURITY_AUDIT priority item 4, no separately numbered roadmap item; both pages are Admin mode. Mandatory general-review and security subagents performed read-only investigation under the owning agent; implementation reviews and all live acceptance checks remain outstanding.

**Part A findings:**
- Audit has separate draft `filters` and `applied: AuditParams`. Search/Enter applies all five fields together and resets page; export must use applied filters, not unsaved inputs or the current 50-row page. Store queries AND org + agent/tool case-insensitive substring matches (SQL wildcard semantics retained) + exact decision + inclusive from/to. **Correction to the audit:** UI datetime-local values are passed unchanged, while both list/export parse RFC3339 only and silently ignore failures. Thus the two UI date dimensions are not actually effective today. Matching list/export needs timezone-qualified serialization and strict shared invalid-bound handling, not blindly reusing this bug.
- **Existing backend export found:** GET /v1/audit/export is already routed and documented; the gap is the page's missing UI plus hardening, not a wholly absent backend. Store fetches 100,001 minimal rows, buffers them, returns at most 100,000 and X-Truncated: true. CSV excludes parameters; it also omits agent_name and formats timestamps to whole seconds. Current OpenAPI describes only required date-only bounds, unlike the actual optional RFC3339 + name/decision filters; apiFetch is JSON-only. One security-review claim of SELECT/Scan mismatch was independently checked and explicitly retracted: both have the same 11 fields, no such defect exists.
- FinOps's three tables use one /v1/finops/summary response (`by_agent`, `by_team`, `by_tool`), complete aggregate arrays with client-side sorting/pagination. Date filtering is server-side on token_usage.recorded_at >= from AND < to; date-only values mean UTC midnight, so the displayed end day is excluded. The picker caps changes to 90 days only in the browser. Team means current gateway_agents.owner, not a separate team entity. CSV should preserve raw numeric values, exact period boundaries, and the existing unknown-pricing caveat; never use abbreviated display strings or a prior range's cached data.
- **Security boundary:** JWT-derived org UUID plus SQL org predicate, same admin/operator/viewer group for list/export/summary; query-string org_id does not override claims. These are org-wide APIs, not workspace-scoped. A workspace member with org role viewer can access them today. The audit table's insert-only RLS policy is not a tenant SELECT guard. FinOps team JOIN uses ga.id = tu.agent_id without ga.org_id = tu.org_id; schema and service-key-only ingestion do not enforce that pairing. A mismatched stored reference could disclose another org's owner label. Recommend the narrow JOIN guard plus a real adversarial fixture, without claiming an ordinary user can write such a reference.
- **Scale correction:** source schema/migrations define audit_log as native monthly RANGE partitions, not a Timescale hypertable (token_usage is the hypertable). No live metadata/row counts confirmed: Docker engine pipe is absent even with escalated docker ps; shared-stack verification is unavailable at investigation time. Existing org/time index helps, but row cap alone does not bound scanned rows, unbounded text bytes, or concurrent request memory. No export query-specific deadline found; HTTP WriteTimeout is not a database-work bound. CSV text is not formula-neutralized and writer errors are ignored.
- **Recommendation for scope decision, not an implementation plan:** reuse/harden existing Audit endpoint with exact applied filters, bounded all-or-error download (suggest 10,000 rows, 20 MiB CSV, 15-second query deadline as initial policy values, not benchmark-derived), clear narrow-filter error and no partial compliance CSV; no raw parameters expansion. Do not COUNT all matches just to detect overflow. FinOps should export the three already-filtered aggregate arrays with honest period/pricing metadata. Formula-safe text, safe filenames, authenticated client download, and no stale/cross-session response reuse are required. Audit export fits AppTopBar.action; FinOps can put a table-choice export control beside its existing date picker. Filter dependence does not require a second toolbar.

No application code changed. B-221 remains BLOCKED at the requested Part A report-before-plan checkpoint; the shared Docker stack must also be available for subsequent mandatory live checks. This is not a completion claim for CSV export.
...
- **B-165 (2026-09-04): logged, HIGH priority, not fixed — eami-agent's remote-config polling has been silently, completely non-functional since it was built.** Two priority clarifications on B-164's own findings, per explicit user direction. **(1) Severity elevation:** the remote-config URL mismatch B-164 Part B found (`eami-agent`'s `FetchConfig` → `eami-collector`'s `ConfigProxyHandler` → a `/v1/agents/{agentId}/config` URL that does not exist in `eami-api`'s router at all, only `/v1/gateway/agents/{agentId}/config` does) is not a minor adjacent footnote — it is a live, ongoing operational failure: every deployed eami-agent has been polling for config updates and silently receiving nothing but 404s this entire time, with both ends treating a non-200 as "not registered yet" and swallowing it (nothing above Debug-level logging anywhere). An admin changing a deployed agent's scan interval/enabled-scanners via the real, working `PUT /v1/gateway/agents/{agentId}/config` endpoint has had **zero effect on any real deployed agent, ever**, with no visible failure signal. Explicitly elevated to **B-129's own severity class** — a real, already-built, currently-100%-non-functional mechanism with zero visible failure signal, same shape as B-129's hot-reload discovery. Given its own B-ID (not left as a sub-note inside B-164) since it's now a real, actionable, urgent item. **(2) Bundling recommendation:** build this together with B-164 Part B's identity-fragmentation fix (`endpoints.agent_id` ↔ `gateway_agents.id`) as ONE brief, not sequenced separately — both are small, both are foundational to Discovery's actual reliability, and they plausibly touch adjacent identity/config code (a real fix here needs to resolve what ID an endpoint should use, which is exactly B-164 Part B's own open question). Full detail in `BACKLOG.md`'s new B-165 entry and B-164's updated Part B note.
- **B-164 (2026-09-04): investigation complete, no code changes — Discovery (Module 1) real-state audit.** Precise, code-grounded answer to "what's genuinely built vs. assumed" for Discovery, given its foundational/first-touchpoint positioning. **Part A:** 12 detection packages exist, only 10 wired into `payload.Build()`; 2 real, working scanners (`scheduled_tasks`, fully implemented for Windows; `file_changes`, a stub) are completely orphaned — never imported outside their own package. Of the 10 wired domains, only 3 (`ai_apps`/`local_models`/`mcp_servers`) are normalized into queryable tables server-side; `ai_processes` and `browser_extensions` are collected and stored but rendered nowhere in the UI (confirmed via grep). A second, entirely separate, dormant HTTP-traffic-observation surface (`discovered_endpoints`/`POST /v1/reports`) has zero producer and zero frontend consumer — not the same thing as the Discover page. **Part B, the significant one:** re-confirmed the identity-fragmentation gap with precise current references — `endpoints.agent_id` is free TEXT (eami-agent's own config/hostname), zero relationship to `gateway_agents.id` (UUID). Found a second, previously undisclosed manifestation of the same root cause while tracing this: eami-agent's remote-config poll calls a URL that doesn't exist in `eami-api`'s router at all (`/v1/agents/{id}/config` vs. the real `/v1/gateway/agents/{agentId}/config`), silently 404ing forever, on top of the ID-space mismatch that would remain even if fixed. Smallest real fix estimated: one nullable FK column + one admin-driven manual link control — a single-session brief, not an epic. **Part C:** B-139 (agentless discovery) reconfirmed genuinely zero-built; one shape-similar but unrelated dormant artifact flagged so it isn't mistaken for a head start. **Part D:** Nodes page reconfirmed exactly as B-080 left it (zero real writer), independently re-verified, confirmed unrelated to shadow-AI Discovery. **Part E recommendation:** close the identity-fragmentation gap first — it's small, foundational, and closes the literal seam between the Discovery and Gateway modules' separate-purchase positioning — before expanding detection breadth or starting the much larger B-139. Full detail in `BACKLOG.md`'s new B-164 entry.
- **B-163 (2026-09-04): DONE — Approvals reclassified out of the sidebar's "Operations" (analysis) group into its own distinct "Approvals" group.** Small, cosmetic, no functional change. **Honest note on the kickoff line's own claimed context:** the task brief referenced "this session's Navigation and Journey Architecture findings" as prior grounding — grepped both this file and `BACKLOG.md` directly and found no such finding logged anywhere under that or any similar name, flagged rather than assumed (same standing practice as this session's own earlier B-155 correction, where a similarly-claimed-already-logged item also turned out not to exist on direct check). Confirmed the real misclassification instead by reading `eami-ui/src/components/layout/Navigation.tsx` directly: `Approvals` sat in `'ops'`/"Operations" alongside `FinOps`/`Memory`/`Paste Detection`/`Audit`/`Alerts` (all after-the-fact analysis pages), despite approve/deny being an in-the-moment action. Mechanism confirmed before building: `NAV_ITEMS`' `group` field + an ordered `NAV_GROUPS` array drive `Sidebar.tsx`'s fully generic render loop — adding a group needed zero `Sidebar.tsx` changes. Added `'approvals'` (label "Approvals" — plain, not inventing new taxonomy for a single-item group given the larger 3-pillar nav revamp stays deferred), positioned between `'gateway'` and `'ops'` (configure → act → analyze). `ApprovalsPage.tsx` completely untouched. Live-verified via the real dev server's live-served module content (browser screenshot unavailable this session, disclosed). Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s new B-163 entry.
- **B-119/B-120/B-142 (2026-09-03): DONE — token-issuance rate-limit hardening + a stale doc-comment fix, all touching the token-issuance auth path, mandatory reviewer + security passes both ran and both found real issues that changed the shipped design.** **B-142** (smallest, done first): re-verified via fresh grep that `registry.LookupByName` still has zero real callers and every real caller uses `LookupByNameAndOrg`; rewrote both functions' doc comments to state the real, current org-scoping design instead of the disproven reasoning that produced B-141. **B-119:** `POST /v1/gateway/tokens`'s per-agent rate limit (B-118's hardcoded 20/60s) is now env-configurable (`config.RateLimitConfig.TokenIssuePerAgent`/`WindowSeconds`), mirroring B-070's `WorkflowRunPerAgent` pattern exactly. **B-120, the significant one — the approved plan's own first design was NOT what shipped.** The approved plan called for a per-remote-IP request-rate limiter (mirroring B-118's shape) guarding the endpoint pre-auth. This session's own mandatory security review found a real, High-severity problem: in this product's actual production topology (B-071, Caddy fronts the gateway directly), every external caller's `r.RemoteAddr` is the proxy's own container IP, so a per-IP limiter there collapses into ONE shared bucket across every org's every agent — meaning a single costless unauthenticated flood could lock out all legitimate token issuance for a full window, **and**, more strikingly, a handful of legitimate agents at their own normal per-agent pace could sum to the shared threshold and trip it with no attacker at all. **Redesigned as a bounded, global, unkeyed CONCURRENCY gate instead** (`preAuthGate`, a buffered-channel semaphore bounding concurrent in-flight `ValidateAndResolveAgent` calls, default 10, env var `TOKEN_ISSUE_PREAUTH_MAX_CONCURRENT`) — this has no fixed window to fill and walk away from, and ordinary legitimate concurrency essentially never saturates it. A second, independent code-review finding (a same-typed-pair constructor-argument transposition risk) was also fixed by bundling the limits into a named `IssueRateLimits` struct. **Live-verified against the real rebuilt/restarted gateway container**, including a deliberate `LOCK TABLE api_keys IN ACCESS EXCLUSIVE MODE` technique to force genuine DB-call concurrency for a real, deterministic proof (plain concurrent `curl` bursts against a fast local Postgres never triggered it — confirmed empirically, not assumed). **New discovered work, both disclosed not fixed:** B-161 (a `RecordIssued` fire-and-forget test race, identical in mechanism to the already-diagnosed B-127) and B-162 (none of B-070's or this session's 6 rate-limit env vars are actually wired through `docker-compose.yml`/`docker-compose.prod.yml`, so the ".env"-based tunability contract these B-IDs establish is currently silent-broken for an operator who doesn't edit the compose file directly). Full detail in `BUILT.md`'s `eami-gateway` section and each B-ID's own `BACKLOG.md` entry.
- **B-122/B-140/B-123/B-126/B-127 (2026-09-03): DONE — test-infrastructure hardening batch, test-only, zero production code touched.** Re-verified all four findings directly against current code before building (all still held exactly as logged). **B-122/B-140 (the largest piece, as flagged before starting):** new shared `eami-gateway/internal/testdb` package (`NewThrowawayPool`) mirrors `eami-api/internal/api/bootstrap_test.go`'s already-proven per-test throwaway-database pattern — create a uniquely-named DB, apply every real `schema/migrations-v2/*.up.sql` file, open a pool, drop it on `t.Cleanup` — adopted across all 11 real-Postgres pool-creation call sites in 9 `eami-gateway` packages, closing B-122's orphaned-`audit_log`-row accumulation and B-140's confirmed hash-chain-continuity race in one motion. Built as one shared, importable package rather than 9 near-identical copies (a plain non-`_test.go` package, since Go can't import another package's `_test.go` files) — the design explicitly approved before building. **B-123:** deleted `internal/workflow/executor_test.go`'s `allowAllRulesRealPolicyID()` (confirmed still byte-identical to the now-fixed `allowAllRules()`), repointed its 2 callers. **B-126:** `internal/workflow/testenv_test.go`'s reconstructed dispatch closure no longer discards `audit.Writer.Write`'s error at its 4 call sites — now logged via a test-local closure mirroring `dispatcher.go`'s `logAuditWriteFailureHook`. **B-127:** new `waitForEpisodeRow` polling helper in `cmd/gateway/dispatcher_test.go` closes the `recordEpisodeHook` detached-goroutine test race by waiting for the episode row synchronously inside the test function (before any `t.Cleanup` fires) rather than touching production code (`recordEpisodeHook`/`episode.Recorder` both out of scope) — proven by a new regression test. **Verification, exactly as the approved plan specified:** `go build`/`go vet` clean; full `go test ./... -count=1` run 5 times in a row, all green, zero flakiness; `TestDispatch_Escalate_Resolution_Expired_WritesSecondRow` (the specific test that motivated B-140) additionally run 20 times in isolation, 20/20 pass; zero leaked throwaway databases confirmed via direct `psql` count. Build order followed exactly as planned (1→4→3→2). Full detail in `BUILT.md`'s `eami-gateway` section and each B-ID's own `BACKLOG.md` entry.
- **B-160 (2026-09-03): logged, EPIC, investigation not started — Enterprise AI Operating System: persona-based workflows. Flagged as the largest, most transformative epic identified this entire session; recommend its own dedicated strategic session, not built casually.** Vision: the platform serves every team in an organization — Compliance, Finance, Data Science, Legal, HR, Procurement, business leaders, executives, and individual employees — not just security/platform admins. **Core architectural insight:** this requires new WORKFLOWS and VIEWS on top of the existing Identity→Action→Tool→Policy→Cost/Audit pipeline, not new underlying data or a parallel system — a direct application of `CLAUDE.md`'s one-spine rule at the epic-scoping level. **Eight personas identified, each needing a genuinely different workflow, not a simplified admin view:** Compliance/Legal (regulatory mapping, attestation workflows, auditor-ready reports — flagged as the single most concrete, immediately valuable piece, recommended as this epic's first standalone investigation); Finance (department-level chargeback/outcome-tied cost reporting, extends FinOps); Data Science/ML (already scoped elsewhere — **B-147**/**B-151**, not new scope here); Business leaders (simplified outcome-only view); Individual employees (**confirmed as B-130's existing scope**, not new); Executives (high-level risk-posture summary); Procurement/Vendor management (extends **B-078**'s data-handling-designation work into a real vendor-risk workflow); HR/People (flagged as legally distinct from the general Compliance/Legal case — AI-in-employment-decisions regimes, NYC Local Law 144 and the EU AI Act's high-risk employment classification specifically named, needing its own regulatory investigation rather than being folded into persona 1). **Elevates, not duplicates, three existing epics/briefs:** B-157 (modular licensing) — persona workflow selection becomes the natural product of B-157's RBAC × Licensing component, not a second access-control system; B-078 (vendor management, Procurement persona's direct dependency); B-130 (confirmed employee-facing home). **Recommendation:** regulatory mapping/compliance reporting as the first real, standalone investigation spun out of this epic — concrete, buildable on data the pipeline already produces, most differentiated piece for the vision's "compliance" pillar. B-ID confirmed free before minting. No code changes. Full detail in `BACKLOG.md`'s new B-160 entry. Counter now stands at **B-161**.
- **B-158/B-159 (2026-09-03): logged, not started — two features sourced from direct, hands-on UX research against TrueFoundry's actual live trial instance (not marketing material).** B-158 (Medium priority): a curated one-click connector/MCP registry — real, popular integrations (GitHub, Slack, Salesforce, Atlassian, etc.) addable with one click, reducing setup friction versus today's only two paths (manual REST-API config, B-044/046; OpenAPI-spec-paste/upload auto-discovery, B-075) — confirmed as a genuine, validated pattern via direct competitor observation, not a speculative idea. B-159 (Low priority, smaller): hosted STDIO-to-HTTP MCP server wrapping — lets a customer host a local/stdio-based MCP server as a real, network-accessible streamable-HTTP server; EAMI has no current equivalent, and the on-prem/air-gapped constraint (ADR-020 Model A) makes "where would this process actually run" a real open question, not yet investigated. **Separately, this same research pass validated existing architecture rather than finding a new gap:** direct UX comparison confirmed EAMI's existing MCP surface (B-061's `tools/list` client capability, B-075's OpenAPI-to-MCP auto-discovery) already matches 2 of TrueFoundry's 4 core "advanced" MCP connection paths — real external validation that those architecture decisions were sound, not just internally reasoned. Both B-IDs confirmed free before minting (counter stood at B-158). No code changes. Full detail in `BACKLOG.md`'s new B-158/B-159 entries. Counter now stands at **B-160**.
- **B-157 (2026-09-02): logged, EPIC, investigation not started — Modular licensing & entitlement system, foundational and business-critical.** Foundation: the three-module product structure (Discovery & Cost Visibility / Gateway / AI Infrastructure) needs a real entitlement system for independent per-customer module enable/disable/billing. **Core design challenge, must be resolved first:** this is an on-prem, potentially air-gapped appliance (ADR-020) — a live "check a billing API" approach compromises the trust story; the real candidate is a cryptographically signed, locally-verified license file, with a genuine open tradeoff (fully offline-only vs. an optional periodic check-in) that must be decided deliberately, not defaulted, since it shapes every other component. **Eight required components logged:** license data model (feature-gating vs. usage-metering kept as separate axes, deliberately not conflated); dispatch-layer enforcement per the one-spine principle (never UI-only, since a direct API call could bypass that); RBAC × Licensing as two composing axes, directly connecting to and expected to resolve **B-113** (global `model_pricing` writable by any org's ordinary admin — a real, already-found gap, more consequential once real billing is at stake), considering a genuine platform-admin/billing-admin tier; visibility/UX (hide-vs-upgrade-prompt) flagged as a real go-to-market decision needing the founder's explicit call, not a technical default; a license admin UI reusing the Settings page pattern; vendor-side license issuance; trial/expiry/grace-period behavior as a real deliberate design; license changes as real audited events per **B-121/B-124/B-125**'s established discipline. **Three additional hard requirements carried forward:** cross-module lineage (a Module 3-trained model must carry a navigable link back to its training run once registered as an `ai_provider` adapter in Module 2 — not automatic from the one-spine principle alone, needs deliberate design); an identity-fragmentation priority escalation (Discover's and Agents' identity models being "entirely disconnected," now more consequential since Discovery and Gateway are separately-purchasable modules and this gap sits at their literal seam) — **honestly flagged that this finding has no logged B-ID anywhere in this backlog as of this session**, checked not assumed, referenced from conversation only; and a UI-consistency mandate (every future module reuses `DataTable`/B-104, the deep-linking mechanism/B-092, and the existing detail-panel convention — sophistication grows in capability, not in interaction pattern). B-ID confirmed free before minting. No code changes. Full detail in `BACKLOG.md`'s new B-157 entry. Counter now stands at **B-158**.
- **B-156 (2026-09-02): investigation complete, no build brief started — pattern-based sensitive-data redaction on the `ai_provider` dispatch path.** Flagged as the single most repeated "important, unbuilt" gap across this session — the core "your data never leaves your building" claim is currently only half-true (every real Claude dispatch sends raw, unredacted content externally). Scoped explicitly to pattern-based detection only (semantic/ML detection is a distinct, later tier genuinely needing the still-parked local-LLM capability). **Where it fits (traced, not assumed):** the true single chokepoint is `aiprovider.Router.Dispatch` (`internal/aiprovider/router.go:98-125`), immediately before its `adapter.Dispatch` call — confirmed there are TWO independent call sites into it (`cmd/gateway/dispatcher.go:669`'s immediate-Allow branch, and `internal/approval/router.go:836`'s escalation-resume path), so a check placed only in the Allow branch would silently miss the resume path. **B-102's `DispatchHook` mechanism does NOT fit and should not be extended** — traced precisely: every hook runs strictly after `outcome` is already built and returns nothing, structurally observational-only, and forking it into a blocking variant would violate B-102's own explicit anti-generalization guard. Recommendation: a plain, small, named interception step inside `Router.Dispatch` itself, not a new hook abstraction. **Library choice, verified via live research:** replicate Microsoft Presidio's proven pattern-design (regex + optional checksum/Luhn validator) natively in Go stdlib `regexp`, zero new dependency — real third-party Go options exist (`go-phileas`, `pii-scrubber`, `pii-extractor`) but aren't worth the dependency for this narrow, well-defined entity set on a compliance-critical path. **Tokenize vs. mask: recommend MASK for v1**, tokenization deliberately deferred as its own dedicated design/security pass given the real new secret-management surface a token↔value map represents. **Audit treatment:** a redaction count belongs as a real, visible, non-hashed `audit_log` column — exactly matching `DataHandling`'s (B-078) own established precedent — never the redacted values. **Per-connector, not global:** `data_handling_designation` (B-078) is the exact template for a config column; `action_paths`' nullable-JSONB pattern is the template for custom regex rules (`gateway_tools.redaction_rules JSONB`). **Security precedent for tokenization, if picked up later:** `toolcreds`/`toolrouter/creds.go`'s application-level AES-256-GCM (key from env var, Postgres never sees plaintext) is the reusable pattern; lifecycle likely in-memory-only for one request/response round-trip (this path is non-streaming today), not yet fully confirmed. **Honest sequencing:** a real multi-brief epic — (1) interception + built-in patterns + mask-only + audit count, (2) per-connector config + custom regex, (3) tokenization as its own dedicated security-reviewed brief if chosen. Every brief here needs mandatory reviewer+security passes as a requirement, not optional, given this is the dispatch path's most security-critical surface. No B-ID for an actual build brief minted yet — Brief 1 would mint its own when picked up, same precedent as B-131. Full detail in `BACKLOG.md`'s new B-156 entry. Counter now stands at **B-157**.
- **B-155 (2026-09-02): logged, EPIC, investigation not started — Conditional workflow branching (structural + LLM-based validation). Correction of an earlier exchange this same session, not a new discussion.** Was discussed in conversation earlier tonight but never actually sent as a task brief; the user initially referred to it as "just logged," which a direct repo check (git log, grep across `BACKLOG.md`/`CONTEXT.md`) showed was not accurate — flagged rather than confirmed, and the user then explicitly asked for it to be logged for real, correcting their own earlier mistake. **Vision:** a workflow step branches to different next steps based on a condition evaluated either (a) against a field already extractable via B-063/064's jq-based extraction mechanism, or (b) by delegating judgment to a real LLM dispatch through the existing `ai_provider` pipeline — the LLM's own constrained answer becomes the typed branch value. **Critical re-examination flagged, not a settled inheritance:** the original "branching is architecturally blocked, no typed connector output schemas" finding (B-058 onward, restated as recently as B-131's own investigation) **predates B-063/064's extraction mechanism** — every intervening entry that cites it (B-064/065/066/067/068/131) repeats it without re-testing it against the extraction mechanism that shipped after. Real investigation must re-examine whether narrow single-field structural conditions are now more tractable, not assume the old conclusion still fully holds. **Real architectural insight:** the LLM-based mode fits the one-spine principle directly — it's just another governed Action (a real `ai_provider` dispatch), not new architecture, and by producing the typed branch value directly it sidesteps the typed-schema problem entirely — likely the more tractable mode, may deserve building first. **Honest scope:** this is a genuine schema/execution-engine topology change (linear → real branching graph), not a UI feature — `workflow_steps`' flat `UNIQUE(workflow_id, step_order)` schema cannot represent it today. **Real open questions, unanswered:** does a branch decision need its own tamper-evident audit record (B-124/125 precedent)? Does an LLM-based condition dispatch need the same TOCTOU protections as any other governed action (B-057 precedent)? How does this interact with `sequential-workflow-designer`'s own `SwitchStep` component (confirmed UI-only, informational, per B-131) once the backend can actually support it? B-ID confirmed free before minting (grepped B-155, none existed, counter correctly stood at B-155) per the standing convention. No code changes. Full detail in `BACKLOG.md`'s new B-155 entry, cross-referenced from B-131's own entry. Counter now stands at **B-156**.
- **B-150/B-151/B-152/B-153/B-154 (2026-09-02): all five logged, investigation not started — from a "TrueFoundry feature-parity initiative" discussion, K8s-native explicitly excluded by deliberate decision.** **K8s-native infrastructure (a competitor-style Kubernetes-operator/CRD deployment model) is deliberately out of scope for this whole initiative, not an oversight** — EAMI's own on-prem/SaaS-hybrid, single-instance-first architecture (ADR-020 Model A, already reasoned as a deliberate correct choice under B-132) is a different, intentional bet, not a gap to close by copying a Kubernetes-native competitor. **Strategic framing locked in for all five and every future brief under them:** "state of the art" means the deepest, most adversarially-proven version of each capability, not feature-checklist parity — this session's own demonstrated security-testing discipline (mandatory reviewer+security passes, live verification, adversarial tests, precise honest disclosure) is the real differentiator in *how* each capability gets built. **B-150** — EPIC, Guardrails: content safety/PII/prompt-injection detection on dispatch, explicitly distinct from redaction (redaction = outbound data protection; guardrails = content safety) — no dedicated redaction epic currently has its own B-ID in this backlog, checked not assumed. Fits B-102's dispatch-hook mechanism directly per the one-spine rule — a new hook, not a new subsystem. **B-151** — EPIC, Model hosting/serving: real GPU-based inference serving (vLLM/Ollama-class) for open-weight/customer-trained models, connecting directly to B-147 (training orchestration) and B-130's self-hosted-model adapter — a served model becomes one more `ai_provider` Adapter registry entry (B-057/B-103), not a separate system; real new GPU-infra complexity flagged, and its relationship to B-147's own already-flagged GPU-visibility component needs confirming, not assuming, when scoped. **B-152** — Feature, unified single-endpoint multi-provider API surface: explicitly gated, not buildable yet — needs 2-3 more provider adapters beyond Claude to mean anything real, a genuine prerequisite per the user's own framing, not a soft suggestion. **B-153** — INVESTIGATION, real performance benchmarking (latency/throughput under realistic load), required before any parity/gap claim vs. competitors' published numbers — real prior concurrency-testing work in this codebase (B-090/B-118/B-128/B-140) is the starting point for a real benchmark methodology, not a synthetic single-request timing. **B-154** — Feature, low priority: application/policy/workflow templates for faster onboarding, a real but least-urgent item of the five. B-ID range confirmed free before minting (grepped B-150 through B-154, none existed, counter correctly stood at B-150) per the standing B-ID-assignment convention. No code changes. Full detail in `BACKLOG.md`'s new B-150 shared strategic-context note and the five entries. Counter now stands at **B-155**.
- **B-148 (2026-09-02): CLOSED — DONE and live-verified, per the user's own explicit close-out.** Core interactivity (add/remove/reorder/click-to-configure, zero backend writes) confirmed working live by the user after the object-identity loop fix (commit `767811d`): flickering at rest is gone, toolbox-add correctly attaches a new step. That loop bug — found by the user's own live testing, diagnosed by reading the actual library source, fixed with a real content comparison rather than a workaround, and verified via a targeted Node test against the real shipped adapter code (7/7 checks) — is part of this brief's real story, not a footnote (see the addendum entry below for the full diagnosis). **Reorder specifically was not separately click-confirmed** in this pass — it shares the byte-identical `onDefinitionChange`→`commit()` path as toolbox-add (confirmed via the core library's `DragStepBehavior.onEnd()` source), so the same fix applies, but this is disclosed honestly as carrying less direct-observation weight than the two paths actually click-tested. **The original "toolbox-drop-during-DOM-swap" theory, flagged at diagnosis time as plausible-but-not-independently-observed, is now strongly corroborated** by the predict-then-observe result (fixing the loop resolved the drop) — still not a frame-by-frame trace, but no longer purely inferred; recorded here explicitly so it isn't silently dropped as a loose end. **New follow-up logged, parked, not urgent: B-149** — general UX refinement of the canvas interactions, specific rough edges not yet pinned down, cross-referenced from B-131's own investigation entry so it's findable later. Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-148 (closed)/B-149 (new) entries. Counter now stands at **B-150**.
- **B-148 addendum (2026-09-02, same session): a real bug found by the user's own live testing, diagnosed precisely and fixed — an infinite destroy/recreate loop in the canvas.** The user tested live and found continuous flicker and a broken toolbox drag-and-drop (new step wouldn't attach). **Diagnosed by reading the actual library source, not guessed:** `sequential-workflow-designer-react` decides whether to fully destroy-and-rebuild its internal `Designer` purely by reference identity (`definition.value === designer.getDefinition()`, confirmed in its own `lib/esm/index.js`), and separately fires `onReady` — treated identically to a real edit — once after *every* `Designer.create()`, not just the first (confirmed in the core package: a one-shot off `onRootComponentUpdated`). `commit()` unconditionally built a brand-new object graph on every call, so every `onReady` echo looked like a fresh edit: `setDefinition` → rebuild → `onReady` fires again → loop, present from mount, independent of user action — fully explaining continuous flicker. The same loop tearing down the whole canvas DOM at rest plausibly (not independently observed) explains the broken drop; separately confirmed via the core library's `DragStepBehavior.onEnd()`/`ToolboxApi.tryDrag()` that toolbox-insert and existing-step-reorder go through the identical vulnerable path, atomically on mouse-release only — so reorder was predicted, not yet user-confirmed, to fail the same way. **Real fix, not a workaround:** `commit()` now does a genuine content comparison (`JSON.stringify(nextValue) === JSON.stringify(definition.value)`) before calling `setDefinition`, skipping it when nothing actually changed — breaking the loop at its source rather than patching a symptom. **Verified per the user's explicit ask for code/test proof, not just an assertion:** a targeted Node test bundles the real, unmodified, shipped `workflowCanvasAdapter.ts` via `esbuild` (confirmed zero runtime dependencies once TypeScript's type-only imports strip out) and replicates `commit()`'s exact content-producing steps plus its literal guard expression — **7/7 checks passed**: the no-op `onReady` echo correctly detected as unchanged; a simulated echo loop converges in 1 iteration (capped at 10 as a sanity bound, proving termination not just a lucky first pass); a genuine field edit still correctly detected, not suppressed; a toolbox-insert-then-echo sequence produces exactly one real change then converges, with the inserted step confirmed present, not silently dropped. **Disclosed, not hidden:** `revalidateExtractionRefs` (unchanged by this fix) wasn't re-bundled/re-exercised in this test — doing so would need `WorkflowsPage.tsx`'s full React/router dependency graph, real import-time-failure risk for a function this fix doesn't touch; the actual React component and the library's own `Designer`/`useEffect` machinery weren't executed either (no jsdom in this project) — this proves the round-trip is idempotent (the property the fix's guard relies on), not a substitute for the user's own re-test. Real `tsc`/`vite build` clean; served module confirmed over HTTP to carry the fix. Full detail in `BUILT.md`'s `eami-ui` section (B-148 addendum) and `BACKLOG.md`'s B-148 entry. Counter unchanged at **B-149**.
- **B-148 (2026-09-02): code/build/grep/review-verified DONE — Workflow Canvas rebuild Brief 2, local interactivity. AC6 (live click-through) still open.** Add/remove/reorder + click-to-configure on the canvas, zero backend writes, mirroring the discontinued canvas epic's own Brief 2 (B-067) boundary. Two small, pre-approved exceptions to `WorkflowsPage.tsx`'s untouched status (same category as B-145's own Eye-icon addition): `export` added to 6 already-existing declarations (`StepConfigPanel`/`StepRow`/`ParamRow`/`revalidateExtractionRefs`/`newStepRow`/`newParamRow`, zero logic change — confirmed via `git diff`) so `StepConfigPanel` could be reused unmodified rather than forked; the Eye-icon's tooltip updated from `"Preview (read-only)"` to `"Open canvas"`. Route path deliberately left unchanged for bookmark/share stability (B-146's own lesson) — only the file/component renamed `WorkflowCanvasPage.tsx`. **Library-native interactions used directly** (toolbox drag-add, context-menu/keyboard delete, drag-reorder, `selectedStepId`/`onSelectedStepIdChanged` for click-to-configure) — confirmed against the library's real `.d.ts` files before writing any code, not assumed, per B-145's own lesson about unverified library behavior; `StepConfigPanel` renders as this page's own overlay (`stepEditor` stays `false`), keeping it byte-for-byte the card editor's own component/props contract. **Mandatory code-review pass (forked subagent) found 2 real state-management bugs, both fixed before shipping:** (1) canvas step labels could bake in a stale/empty tools list if a mutation happened before `useTools()` first resolved (unlike the card editor's fresh-every-render resolution) — fixed by gating the one-time definition seed on tools readiness too; a related blank-loading-state gap this exposed was found and fixed in the same pass. (2) `realStepIds` was recomputed on every `workflow` query reference change instead of captured once — a real background-refetch scenario (network blip + a concurrent card-editor step removal elsewhere) could have silently reclassified an in-use canvas step as "not real," invalidating extraction refs with no user action — fixed by moving it into the same one-time seed as `definition`. **Mandatory security review (forked subagent): zero HIGH/MEDIUM findings.** AC1-3/AC5 verified by grep + real `tsc`/`vite build` + served-module HTTP checks; **AC6 (a real, live click-through) explicitly NOT done this session** — `claude-in-chrome` unavailable, disclosed as open rather than assumed passing, same honesty standard as every other frontend brief this session. Two real seeded workflows (`b059-live-verify`, `b063-live-verify`) ready for the user's own testing; dev stack live-confirmed running/healthy and a real bcrypt test password set on `dev@example.com` for login (`EamiCanvasTest#2026`, told to the user directly). Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-148 entry. Counter now stands at **B-149**.
- **B-146 (2026-09-02): DONE — session persistence on hard reload, redirect-back-after-login.** `authStore.ts` gained real `localStorage` persistence (Zustand `persist`), chosen over an HttpOnly cookie because the cookie option needed `eami-api` changes this fix's own scope (frontend-only) explicitly forbade — a hard scope constraint, not a preference, documented in-code and disclosed alongside the real XSS tradeoff (zero `dangerouslySetInnerHTML` repo-wide, B-118's short access-token TTL bounds exposure — neither eliminates the risk). AC2 (redirect back to the originally-requested URL) achieved without touching `AppShell.tsx`/`router.tsx` (both out of `MAY-MODIFY` scope) via a module-load-time capture of `window.location` in `authStore.ts`, consumed once by `LoginPage.tsx`. **A real wrong assumption caught by live verification and corrected before shipping:** the approved plan included a pre-emptive client-side JWT-expiry check on the refresh token, to avoid a UX flash for a dead session — live verification against the real running `eami-api` showed `refresh_token` is a cryptographically random **opaque** value (`auth.go`'s `IssueRefreshToken`), not a JWT, confirmed both via a real `/v1/auth/login` call and in source; the planned decode would have thrown on every real token and forced a logout on every hard reload — the exact bug this brief exists to fix. Removed entirely rather than patched around; the existing, unmodified `lib/auth.ts` 401→refresh→`logout()` flow is relied on instead, since only the server can know a refresh token is actually dead — confirmed live via `curl` against `/v1/auth/refresh` (garbage token → real `401`; valid token → real fresh pair). **Live verification, honestly scoped:** `claude-in-chrome` declined again this session (no browser click-through); in its place, real `tsc`/`vite build` (clean, run twice), the real dev container rebuilt and its served module confirmed correct over HTTP, and — the centerpiece — **the actual shipped, unmodified `authStore.ts` bundled via `esbuild` and executed under three Node-process scenarios with stubbed `window`/`localStorage`**, against a real access/refresh token pair obtained from the real running backend (dev seed user's password reset via direct `psql` for login purposes only, not an app-code change): 11/11 assertions passed (rehydration, redirect-path capture/consumption, `/login` exemption, `logout()` writing through to persisted storage). What was not verified: actual browser rendering — disclosed, not glossed over. Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-146 (DONE) entry. Counter unchanged at **B-148**.
- **B-130 (2026-09-02): RAGFlow candidate framework logged under the RAG addendum, with license research — canonical copy here, cross-referenced (not duplicated) from B-147.** RAGFlow's default storage backend, Elasticsearch, is triple-licensed (AGPLv3/SSPL/Elastic License 2.0) as of August 2024; current research confirms embedding it unmodified as an internal backend (not resold as a hosted service) carries effectively zero licensing risk per informed industry sources, with the one real constraint being never modifying Elasticsearch's own source (which would trigger AGPL's copyleft obligation). **Recommendation: don't use Elasticsearch anyway** — two clean, zero-ambiguity Apache 2.0 alternatives exist: RAGFlow's own "Infinity" database (same maintainer, purpose-built for hybrid vector/full-text search) or OpenSearch (Linux Foundation-governed Apache 2.0 fork). For a compliance/governance-focused product, removing licensing ambiguity outweighs any marginal Elasticsearch capability; if RAGFlow is adopted, configure `DOC_ENGINE=infinity`. Kept as one canonical entry under B-130 (RAG's natural home) with a pointer from B-147's dataset-management item, deliberately avoiding the two-copies-drift risk this session has guarded against with every other cross-referenced item tonight. No code changes. Full detail in `BACKLOG.md`'s B-130 entry. Counter unchanged at **B-148**.
- **B-147 (2026-09-02): permanent license reference section added, no code changes.** Researched (not legal advice, verify with counsel before shipping commercially) license terms for the tooling this epic's investigation has named so far: **Axolotl** and **Ludwig**, both Apache 2.0 — commercial/proprietary use allowed freely, requires preserving copyright/license notices and stating changes if modified, includes an explicit patent grant from contributors plus a patent-retaliation clause (EAMI's license terminates only if EAMI itself sues a contributor over a related patent). **Gitleaks**: the core detection engine/CLI is MIT (simpler — use/modify/embed freely, only the copyright notice must be preserved); flagged an important distinction — `gitleaks-action` (the separate GitHub Actions CI wrapper product) requires a paid license for organizational use, which does NOT apply if only the core engine is embedded as a dataset-scanning step rather than the Action itself; also flagged that Gitleaks is now feature-complete/maintenance-only, with active development moved to a successor ("Betterleaks") whose license has not yet been checked and needs separate verification before serious consideration. **Practical obligation logged:** if any of these ship in the product, a real `THIRD_PARTY_LICENSES` file (or equivalent) is needed, listing each dependency's license text — routine, low-cost, same compliance pattern already handled for `sequential-workflow-designer`'s MIT license (B-131). No code changes, investigation still not started. Full detail in `BACKLOG.md`'s B-147 entry. Counter unchanged at **B-148**.
- **B-147 (2026-09-02): logged, EPIC, investigation not started.** Customer-controlled LLM training orchestration — deliberately NOT an ML training engine built by EAMI, but a state-of-the-art orchestration/management layer for training the customer runs entirely on their own infrastructure, matching the on-prem model (ADR-020 Model A). Real, current framework identified to orchestrate rather than reinvent: Axolotl (Apache 2.0, actively maintained, YAML-config-driven, LoRA/QLoRA/full fine-tuning, real multi-GPU/multi-node support) — it explicitly leaves GPU ops/monitoring/serving to the wrapping platform, which is exactly EAMI's opportunity. Six real components identified: (1) guided UI generating correct training configs and kicking off jobs, (2) dataset management connecting naturally to the RAG capability's (B-130 addendum) document pipeline, (3) GPU resource visibility/allocation — genuinely new infra, (4) experiment tracking, honestly scoped short of full MLOps-platform depth, (5) model registry/deployment — the key integration point, wiring a newly-trained model into the SAME `ai_provider` Adapter pattern (B-057/B-103) B-130 already reuses for self-hosted models, (6) vendor-hosted fine-tuning-as-a-service support where offered, smaller, same adapter pattern, needs per-vendor research when scoped. **Honest positioning locked in:** RAG is the faster/cheaper answer for injecting knowledge; fine-tuning is the right tool for changing behavior/style — this epic should never be pitched as a RAG replacement. **Scale:** genuinely large, comparable to B-130 — explicitly not a small add-on to B-130 or RAG, though it pairs naturally with both. Recommended MVP framing (not yet a real brief): single framework (Axolotl), single-node LoRA/QLoRA first, defer distributed multi-node training to a later phase — mirrors this repo's own incremental-epic discipline. B-ID confirmed free before minting: grepped `BACKLOG.md` for an open item at this number (none existed — B-146 was already taken by the auth-session bug, counter correctly stood at B-147) per the standing B-ID-assignment convention. No code changes. Full detail in `BACKLOG.md`'s new B-147 entry. Counter now stands at **B-148**.
- **B-130 (2026-09-02): scope addendum, no code changes — dynamic agentic loop confirmed in-epic, RAG added in-epic, fine-tuning explicitly split into its own future epic.** The "dynamic agentic loop" pattern discussed in Thread A (an LLM autonomously deciding mid-conversation to call a tool — Thread A's "Model 2", originally deferred there as 3-5x larger than what that thread actually built) is confirmed as real, in-scope B-130 work, not a separate idea: a governed chat client that can't loop on its own tool calls isn't the target experience. Two implementation paths identified, not combined into one estimate: **(a)** the gateway itself orchestrates the loop — call the LLM, inspect for tool-use, dispatch through the existing governed pipeline, feed the result back, repeat — self-contained, does **NOT** require B-076 resolved; **(b)** true remote-MCP integration, where Anthropic's own infrastructure calls EAMI's MCP server directly, **DOES** require B-076 resolved (still QUEUED, not investigated) and additionally depends on an external, unverified dependency (Anthropic's actual remote-MCP behavior, outside this repo's control). (a) recommended as the more tractable, self-contained path. **New open design question, not resolved by anything already built:** if a mid-loop tool call gets escalated for human approval, does the whole agentic loop block waiting on it, or something else — needs real design. Separately, **RAG added as a real, near-term-buildable capability within this epic**: architecturally "one more governed tool call" (a vector-search connector in the same dispatch pipeline already proven), making the existing "context lives within the gateway" requirement a natural consequence rather than new complexity — but a real embedding/vector-store ingestion pipeline does not exist anywhere in this system today, flagged as genuinely new infra to confirm before assuming it's free (pgvector's presence in `eami-api`'s Postgres per `ARCHITECTURE.md` is not the same as a built ingestion pipeline). **LLM training/fine-tuning explicitly logged as its own separate, much-later, much-larger epic — not part of B-130**, pending clarification of which of three genuinely different things it means: (a) full weight retraining (likely infeasible against frontier models, needs research into viable self-hostable open-weight models), (b) provider-API-supported fine-tuning where available (smaller, provider-specific, needs current research), or (c) what's usually actually meant in practice — either RAG (item above) or lightweight per-customer system-prompt customization, much smaller and likely already trivially achievable. No code changes, investigation still not started. Full detail in `BACKLOG.md`'s B-130 entry (2026-09-02 addendum). Counter unchanged at **B-146**.
- **B-145 (2026-09-01, closed out): a real blank-canvas bug found by actual human visual verification, fixed in two passes; a real false alarm ruled out by evidence; a real permanent nav entry point added.** After B-145's initial "verified" build (real `tsc`/`vite build` clean, real data confirmed, module serving confirmed) was actually looked at in a browser, the canvas was completely blank. Traced through the library's own source (not guessed): `sequential-workflow-designer` measures real layout pixels, its own CSS is `.sqd-designer{height:100%}`, and the div its React wrapper mounts into has zero CSS of its own — fully dependent on its container having a genuinely definite size. This page's `flex h-full flex-col` wrapper was the first in the app needing to fill remaining viewport height, and relied on a percentage cascade through `AppShell`'s shared `main` with no `min-height:0` anywhere in it. **Pass 1** (`h-full`→`h-[75vh]`) fixed height, confirmed via the user's own DevTools measurement (`0`→`550.5px`) — but width stayed exactly `0` (real DOM mounted, `childElementCount:1`, genuinely zero-size, not a mount failure). **Pass 2**: `flex` only auto-stretches the cross axis (height) by default — swapped for `grid`, whose default `place-items:stretch` covers both axes for an unstyled child. Live-confirmed by the user afterward: real flowchart, distinct step boxes. **Separately, a real false alarm ruled out through direct evidence, not assumption**, and deliberately kept out of the actual-fix record above: an uncaught `reportAllChanges`/`startTime` browser error traced to Google's `web-vitals` library's exact public API shape — but confirmed absent from this project's entire dependency tree (`package-lock.json` and all of `node_modules` grepped, zero matches); the only `startTime`+`PerformanceObserver` code anywhere in the repo is `rollup`'s own Node-side build-time profiling, which never reaches the browser. Persisted in Incognito too — most consistent with an unrelated browser extension, correctly left unfixed since there was nothing in this codebase to fix. **A real differential test** (`b059-live-verify`, genuinely different steps, vs. the initial `b063-live-verify`, whose two real steps happen to be identical) confirmed `workflowCanvasAdapter.ts` had no bug — an earlier "duplicate label" report traced to the user comparing two different workflows, not an adapter defect. **Closed out**: a temporary DevTools-verification button on `DashboardPage.tsx` (added, used, then fully reverted — confirmed zero diff against the committed file) replaced by one small, permanent, explicitly-requested `Eye`-icon "Preview (read-only)" link in `WorkflowsPage.tsx`'s existing row-actions column — purely additive, zero changes to any existing add/edit/delete/reorder/save logic. Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-145 entry (updated in place, still DONE).
- **B-146 (2026-09-01): logged, not fixed — auth session doesn't survive a hard page reload; login never redirects back to the originally-requested page.** Found live while manually verifying B-145: `authStore.ts` holds tokens in-memory only (no persist middleware, no localStorage, no cookie), so any hard navigation (refresh, bookmark, shared link, direct URL paste) discards the session; `LoginPage.tsx` always redirects to `/dashboard` afterward, never back to where the user was headed. Surfaced by B-145's new, deliberately-unlinked `canvas-preview` route (only reachable via URL paste = hard nav) but **not a B-145 bug and not route-specific** — identical behavior on every route if reached the same way. Real, live impact beyond dev inconvenience: a customer refreshing their browser, or opening a bookmarked/shared deep link (including B-092's own Slack-approval-notification links), loses their session and lands on the dashboard instead. Two independent fix halves sketched, not decided: token persistence (localStorage via Zustand's `persist` vs. an HttpOnly cookie — a real security tradeoff, not defaulted to either) and a redirect-back-after-login flow (smaller, independent, worth shipping even if persistence is deferred). Full detail in `BACKLOG.md`'s B-146 entry.
- **B-145 (2026-09-01): DONE — Workflow Canvas rebuild Brief 1, `sequential-workflow-designer` read-only rendering.** First build brief off B-131's investigation. New route `/gateway/workflows/:id/canvas-preview` (`WorkflowCanvasPreviewPage.tsx`/`workflowCanvasAdapter.ts`), zero edits to `WorkflowsPage.tsx` (explicitly out of MAY-MODIFY scope, not just functionally unaffected), `sequential-workflow-machine` (the execution engine) explicitly not adopted. **Two premises the investigation flagged as needing empirical confirmation, both resolved by actually building, not by re-reading docs:** (1) the free-tier custom-step-type ambiguity (README lists it under Pro examples, Task's own doc page shows it free, pricing page's real Pro list doesn't include it) — built a real custom step type (`eami-connector-action`) with real EAMI data + a custom icon via the free `steps.iconUrlProvider`, confirmed working with zero Pro package installed; (2) Tailwind/Vite compatibility — confirmed via the real Vite dev server serving the library's own CSS with zero errors, no JIT conflict (Tailwind never scans `node_modules` CSS). Every interactive surface disabled individually (`toolboxConfiguration`/`controlBar`/`contextMenu`/`keyboard`/`rootEditor`/`stepEditor` all `false`, plus `isReadonly`), zero mutation hooks anywhere in the two new files (confirmed by grep) — the new view's only backend call is the one necessary read. Real measured bundle delta: +29.3 kB gzipped JS, +1.3 kB gzipped CSS. **Live verification, honestly scoped:** real `tsc`/`vite build` clean; `eami-ui`'s dev container rebuilt (node_modules baked into the image, not bind-mounted) and the new page/adapter modules confirmed serving with zero errors through the real dev server, `WorkflowsPage.tsx` re-confirmed unaffected, a real dev-DB workflow confirmed available for use. `claude-in-chrome` was offered and declined this session, so actual browser-rendered DOM was not visually confirmed — disclosed plainly rather than assumed. Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-145 (DONE) entry. Counter now stands at **B-146**.
- **B-144 (2026-09-01): DONE — optional offsite backup transport, closing B-029's original "biggest gap."** Grounded in ADR-020 Model A (EAMI ships the mechanism, customer controls the destination and every credential): new `scripts/backup-offsite.sh`/`restore-offsite.sh` + `docker/eami-backup/Dockerfile` (adds `age` + `rclone`). **Tooling reasoning:** `rclone` over `restic` (restic owns its own repo format/encryption, conflicts with customer-holds-the-key), `age` over GPG (same "small, direct tool" fit). **Gating is structural, not a flag:** no-ops unless `OFFSITE_AGE_RECIPIENT`/`OFFSITE_RCLONE_REMOTE`/a real `secrets/rclone.conf` are all present — `backup-db.sh` gained one small additive block at its end, core dump/retention logic untouched. Encryption happens before any network access; the encrypting side never holds a decryption key. **AC5 resolved explicitly:** whole-file encryption of the complete dump is sufficient for the JSONB `parameters`/`audit_log`-content risk B-134 flagged — confirmed empirically (`strings`/`pg_restore -f -` found real plaintext occurrences in the raw dump, zero in the `.age` file). **Real live verification, non-destructive:** a genuine MinIO container (real S3 API) as the test destination, a real `age` keypair with the private key never touching the backup-producing container. All 5 acceptance criteria confirmed live: disabled-by-default behavior unchanged; decryption fails without/with-wrong identity and succeeds with the correct one; the encrypted file actually lands in MinIO via customer-only credentials; a full restore drill from a second, completely separate isolated stack (simulating total host loss) recovers via `restore-offsite.sh` → the unmodified B-143 `restore-db.sh` and passes the full B-134/B-143 regression checklist (hash chain, workflow/approval state, credential decryption) with zero regressions. All test resources (2 isolated stacks, MinIO, a dedicated network) torn down after, zero impact to the real dev stack. Full detail in `BUILT.md`'s Cross-cutting section and `BACKLOG.md`'s B-144 (DONE) entry.
- **B-143 (2026-09-01): DONE — restore-time hypertable FK constraint loss fixed, `RECOVERY.md`'s stale initdb claim corrected.** B-134's fresh restore drill found `pg_restore` always generates `ALTER TABLE ONLY ...`, which TimescaleDB hypertables reject — B-029 already found this for `--clean`'s `DROP CONSTRAINT`; this brief found the same root cause also breaks plain `ADD CONSTRAINT`, silently dropping 3 FK constraints on `gateway_node_metrics`/`paste_events` (row data unaffected, enforcement gap only). **New `scripts/restore-db.sh`, deliberately generic:** wraps the schema-wipe + `pg_restore` steps, parses `pg_restore`'s stderr for the "hypertable" error text paired with a failed `ALTER TABLE ONLY ... ADD CONSTRAINT`, strips `ONLY`, re-applies — covers a future hypertable-FK table automatically, not hardcoded to today's two. **Fails loudly by design:** any error not matching that exact shape is left alone and reported, script exits non-zero rather than guessing — verified against a synthetic fixture mixing 1 real hypertable error with 2 unrelated ones, correctly repairing only the 1. `docker-compose.yml`/`docker-compose.prod.yml` updated to mount the new script into `eami-backup` (same pattern as `backup-db.sh`); `.gitattributes` given the same `eol=lf` pinning for the same bind-mount-into-Alpine reason. `RECOVERY.md` corrected: no `docker-entrypoint-initdb.d/schema.sql` auto-apply since B-051 moved schema ownership to `migrate`; `pg_restore` recreates the full schema from the dump regardless, so `migrate` running first isn't required for restore. **Live-verified, same non-destructive isolated-stack methodology as B-134** (backed up the real running dev stack read-only, restored into an isolated `docker compose -p b143dr` stack via the real documented `eami-backup` command, torn down after): all 4 constraints repaired automatically, `pg_constraint` for `gateway_node_metrics`/`paste_events` came back byte-identical to the live source (0-line `diff`). Full B-134 regression checklist re-run clean: hash chain still 0 bad hashes/1636 rows, workflow/approval/credential-store counts identical to live, all 3 real credentials still decrypt. B-144 (offsite mechanism) updated — no longer blocked on a known-gap restore procedure. Full detail in `BUILT.md`'s Cross-cutting section and `BACKLOG.md`'s B-143 (DONE) entry.
- **B-134 (2026-09-01): investigation complete, no code changes — split into B-143 (fix) + B-144 (offsite design).** Two-part investigation-only brief: (A) re-verify B-029's backup mechanism still covers everything current, (B) run a genuinely fresh restore test against current schema, not a re-read of the old one. **Part A:** confirmed the backup mechanism (`pg_dump -Fc`, full-database, no table allowlist) automatically covers every table added since B-029 — `ai_token_events`, `agent_lifecycle_events`, all workflow tables — zero maintenance burden. One doc-accuracy gap found: `RECOVERY.md` still claims `docker-entrypoint-initdb.d/schema.sql` auto-applies on a fresh volume, confirmed empirically false since B-051 moved schema ownership to the `migrate` service — a truly fresh volume today has zero tables (`\dt` returns nothing) until `migrate` runs. **Part B, real execution:** backed up the real running dev stack (non-destructive), restored into a fully isolated `docker compose -p b134dr` stack (separate volumes/network, torn down after — confirmed zero impact via a full `diff` of the restored vs. live `audit_log` table, 0 differences). **Hash chain: 0 bad hashes across all 1636 rows**, independently recomputed with a fresh standalone verifier matching `writer.go`'s exact formula — restore preserves tamper-evidence perfectly. 205 "broken link" rows found, but proven pre-existing (identical in both restored and untouched-live copies via the same diff), not restore-caused — consistent with B-140's already-tracked concurrent-writer race, not this brief's to fix. Workflow/approval/credential-store counts all matched live exactly; all 3 real encrypted credential blobs decrypted successfully post-restore with the real key. **Real restore bug found, logged as B-143 (High):** `pg_restore` fails to apply 4 FK constraints on hypertables (`gateway_node_metrics`, `paste_events`) with `ERROR: ONLY option not supported on hypertable operations` — same bug family as B-029's original `--clean` finding, now on `ADD CONSTRAINT` instead of `DROP`. Row data unaffected; referential-integrity enforcement is the gap. Fix confirmed working by hand (drop `ONLY`). **Offsite design recommendation, logged as B-144 (EPIC, medium):** grounded in ADR-020 Model A ("zero data ever reaches EAMI-operated infrastructure") — rules out any EAMI-operated cloud target; recommended an opt-in customer-configured remote target (S3/SFTP via `rclone`/`restic`), disabled by default, dump encrypted before leaving the host. Deliberately kept separate from B-143 so the small, immediately-buildable restore fix doesn't wait on the larger offsite-mechanism design. Full detail in `BACKLOG.md`'s B-134 (DONE)/B-143 (new)/B-144 (new) entries. Counter now stands at **B-145**.
- **B-141 (2026-09-01): DONE — JWT org-binding + org-scoped identity resolution, closing the confirmed cross-tenant episode-content read.** Investigation traced all 4 unscoped `registry.LookupByName` call sites precisely and confirmed none had an org_id available before the lookup itself (the lookup is what discovers it) — meaning JWT org-binding (B5) and per-call-site scoping (B6) were never independent alternatives; B5 is the structural prerequisite for B6. Also traced B-042's original "safe by construction" reasoning precisely: two call sites (`workflow/http.go` B-059, `workflow/ratelimit.go` B-070) were added *after* B-042 already existed as documented precedent in the same file, neither reusing it. User approved the plan; built exactly it. **Shipped:** `identity.Claims`/`IssueRequest` gained `OrgID` (additive, same pattern as B-116; `IssueRequest.OrgID` has `json:"-"`, confirmed empirically unspoofable via request body); `HandleIssue` sets it from the already org-scoped `key.OrgID`; all 4 call sites (`mcp/handler.go` ServeSSE, `workflow/http.go` HandleRun, `workflow/ratelimit.go`, `episode/http.go` authenticateCaller) switched to the pre-existing, untouched `LookupByNameAndOrg` (B-042). **Hard cutover** (pre-fix tokens rejected with 401) at 3 of 4 sites; `RateLimitRunMiddleware` deliberately stays best-effort pass-through, confirmed safe by both mandatory review passes tracing `main.go`'s real wiring (`HandleRun` is `next` directly, its own independent check is the real boundary). **Mandatory reviewer + security passes both clean** — zero correctness/security defects in the fix; both independently found the same single stale-doc-comment issue in `registry.go` (outside this brief's scope), logged fresh as **B-142** (Low). **Test coverage — three real-Postgres surface tests matching B-128's own two-org standard:** `TestServeSSE_OrgScoping_*`/`TestHandleRun_OrgScoping_*`/`TestListEpisodes_OrgScoping_*`, the last one doubling as AC2's negative control — **empirically confirmed to catch the bug** (fix manually reverted, test re-run and confirmed to fail with the exact predicted leak, fix restored, test re-confirmed passing), not just asserted. A pre-existing test's context-smuggling device (`ctxOrgResolver`) became genuinely obsolete and was deleted, replaced with two real tokens carrying real `OrgID` claims — a strictly stronger test, not a regression. Full `go test ./...` green across `eami-gateway`, zero regressions, including targeted re-runs of B-128/B-098/B-042's own tests. Live-verified against real Postgres and the rebuilt/restarted gateway container. Full detail in `BUILT.md`'s `eami-gateway` section and `BACKLOG.md`'s B-141 (DONE)/B-142 (new) entries. Counter now stands at **B-143**.
...
## Active decision thread (2026-09-23, older still) — `rheoARC_Roadmap_Enterprise_AI_Platform.md` committed to the repo root, closing the roadmap-read rule's own enforcement gap. The rule (added in the immediately preceding entry) initially referenced a file that did not exist anywhere on this machine — confirmed via a full local filesystem search (repo, Desktop/Downloads/Documents), flagged explicitly rather than papered over or fabricated. A subsequent handoff attempt from the founder referenced "the roadmap content from my earlier message," but no such content had actually been sent in any message — flagged that discrepancy directly a second time rather than inventing Horizons/numbered items to fill the gap, matching this session's own standing no-fabrication discipline. The founder then supplied the real content directly (previously discussed only in a separate chat context, never written into this project's files); saved verbatim, committed, pushed. Four Horizons: 0 (foundation, shipped and adversarially proven — correctness/security bar, not feature maturity), 1 (known-gap closure — CMDB completion, SSO, agentless discovery, observability, public API, connector registry, onboarding templates, B-213/B-218/etc.), 2 (the agentic layer — model hosting, RAG, guardrails, orchestration, autonomy safeguards, the Chat Engine, training orchestration, multi-agent coordination, strictly dependency-sequenced), 3 (enterprise scale-readiness). `CLAUDE.md`'s own rule-text caveat updated from "not yet enforceable" to a record of why the rule exists in this exact form.

## Active decision thread (2026-09-23, older still) — Roadmap discipline added as a new standing rule in `CLAUDE.md`'s Conventions section, given the same weight as the `aiprovider.Adapter` pattern, the dispatch-hook convergence rule, and the `DESIGN_SYSTEM.md` mandate: every future epic/major-feature investigation must map to a specific Horizon and numbered item in `rheoARC_Roadmap_Enterprise_AI_Platform.md`, confirmed explicitly at kickoff, or the mismatch must be flagged and confirmed before proceeding — never silently started as net-new, unsequenced work. At the time this rule was added, the referenced file did not exist anywhere on this machine (see the immediately following entry for how that gap closed) — disclosed directly in the rule's own text rather than left as a silent broken reference.

## Active decision thread (2026-09-23, older) — B-217 built: B-196 increment 1, real CMDB asset list (endpoints + gateway_agents + gateway_tools only, no fabricated relationships) — the first real code shipped for the CMDB epic, following a dedicated Part A/B/C investigation reported before any implementation. Investigation confirmed: real data exists for endpoints (5)/gateway_agents (12)/gateway_tools (4), all queryable; `gateway_nodes` (0 rows, no real writer per B-080) excluded from this increment's own scope; the epic's "AI Workload" CI category (Deployed LLM Instance, Training Job) has zero real backend anywhere — B-151/B-147 both still zero-build, stays design-only; `workspace_id` is real and schema-enforced on endpoints/gateway_agents but `gateway_tools`/`gateway_nodes` have no such column at all; the only real multi-hop relationship today is B-200's own agent-centric `GetAgentConnections`; zero real admin-configurable CI-classification backend exists anywhere (grepped, confirmed by absence). Also corrected a real error in the epic's own prior (2026-09-20) `BACKLOG.md` extension note — `models`/`cloud_clients` aren't real table names; the real tables are `endpoint_ai_apps`/`endpoint_mcp_servers`/`endpoint_model_files` — fixed in place, not just flagged. **Part A decision, reported before building:** `gateway_tools` rows get a visually distinct "Not workspace-scoped" badge (dashed border, muted gray), never the same "Global floor" gray `ScopeBadge` already uses for a real-but-null workspace assignment — conflating the two would falsely imply a scoping mechanism `gateway_tools` doesn't have (`DESIGN_SYSTEM.md` §7.4). **What shipped:** `GET /v1/gateway/agents`/`{agentId}` and `GET /v1/endpoints`/`{endpointId}` now merge real `workspace_id`/`workspace_name` (B-214's merge-not-extend pattern); new `pages/cmdb/AssetsPage.tsx` merges the 3 already-existing list endpoints into one categorized table with static taxonomy tags (not the admin-configurable panel `DESIGN_SYSTEM.md` §7.2 describes, explicitly deferred) and workspace badges; endpoint rows reuse `DiscoverPage.tsx`'s existing `EndpointDrawer` directly, agent rows open a new lightweight panel reusing B-200's `useAgentConnections` hook as a plain list (deliberately not the relationship-graph canvas), tool rows open a new read-only panel with no relationship section at all (no real relationship data exists for tools — a fabricated empty section would imply otherwise). New "Assets" nav entry flagged against `CLAUDE.md`'s one-spine rule in a code comment and judged a legitimate exception (read-only normalization layer over 3 of the six core pages' own data, no new identity/audit/policy mechanism, a long-scoped founder-approved epic with its own design-canvas backing). **Mandatory code review found 2 real issues, both fixed:** `GetAgent`/`GetAgentEndpoint` originally reused the list-oriented workspace-lookup helpers for a single row — O(all workspace-scoped rows in the org) instead of O(1), deviating from `GetPolicy`'s own correct B-214 precedent — fixed with dedicated single-ID-scoped helpers; `AssetsPage.tsx` silently capped endpoints at 200 rows with zero UI indication — fixed with an explicit truncation banner comparing the real `meta.total` against what actually rendered. **Mandatory security review: zero real findings** (new raw SQL confirmed parameterized and org-scoped; one non-blocking design note that the `workspaces` JOIN doesn't independently re-check `w.org_id`, not currently exploitable since no live write path sets these columns yet). Live-verified with a real Playwright browser: one real existing endpoint and one real existing agent assigned to a real pre-existing workspace directly via `psql`; `/assets` rendered exactly 18 rows, cross-checked directly against `psql`'s own real count for that org (5+9+4), with exactly 2 real workspace-name badges, exactly 12 "Global floor," exactly 4 "Not workspace-scoped" — all exact-count cross-checks against real data, zero console/page errors. New B-218 logged (not fixed): the `gateway_tools`/`gateway_nodes` `workspace_id` schema gap itself, small and mechanical whenever actually needed. Full detail in `BUILT.md`'s `eami-api` section and `BACKLOG.md`'s new B-217/B-218 entries.

## Active decision thread (2026-09-23, older) — B-216 built: the last two honest gaps in the Workspaces epic closed — real post-login routing for a pure workspace member + a real "My Workspaces" nav entry for two-hat users (Part 1), plus a live-verified confirmation that `WorkspaceShell`'s frontend already honestly reflects the backend RBAC boundary (Part 2, no code changed). Found reviewing the otherwise-complete epic (B-207/208/209/212/214/215): a pure workspace member (role=`viewer` + a real membership) landed on `/dashboard` after login with nothing to do there, and a two-hat user (real org role + real membership) had zero visible entry point into Workspace mode from Admin's own navigation — Profile's membership list, two clicks from primary nav, was the only path in. Part 1 investigation confirmed `users.role` has a `NOT NULL CHECK` (`admin/operator/approver/viewer`) — no DB state of "no org-level role" — so `role === 'viewer'` was used as the real floor, while `'approver'` (a genuine, narrower Admin-mode purpose) was deliberately left unchanged and disclosed separately rather than folded in. Part 2 investigation traced `WorkspacePoliciesPage.tsx`'s `isAdmin` precisely from the real current code (not from B-215's own prior summary): already resolved from the real per-workspace role via `useMyWorkspaces()`, already gating every mutation affordance — no gap existed, no code changed, only live verification. **Mandatory code review found 4 real issues, all fixed:** a stale cross-account react-query cache leak on logout (`queryClient.clear()` had never been called — a second user logging in within the same SPA session could momentarily see the first user's cached data, e.g. a stale "My Workspaces" sidebar entry — now cleared in `authStore.ts`'s `logout()`, closing this for every current/future query hook at once); the new nav entry's tension with `CLAUDE.md`'s one-spine rule, disclosed and reasoned through explicitly in a code comment rather than silently resolved (it's an entry point into an already-separate, already-reviewed mode, not a new identity/audit/policy silo); hand-duplicated `NavLink` markup in `Sidebar.tsx`'s collapsed/expanded branches, eliminated by extending `Navigation.tsx`'s `NavItem` with a `conditional?: 'hasWorkspaces'` field and folding "My Workspaces" into the existing generic `NAV_ITEMS`/`NAV_GROUPS` render loops; and a duplicate `GET /v1/workspaces/mine` fetch on the viewer-login path (`LoginPage.tsx`'s raw `apiFetch` bypassed react-query's cache, so `WorkspaceShell`'s own `useMyWorkspaces()` immediately re-fetched on the next render) — fixed by priming the cache via `queryClient.setQueryData(['workspaces', 'mine'], mine)` right after the fetch. `tsc`/`vite build` both clean after all 4 fixes. All 4 Part-1 ACs and both Part-2 cases re-verified live with a real Playwright browser after the fixes, including a dedicated logout cache-clear check (user A with a real membership logs out via the real `UserMenu` button, user B with zero memberships logs in immediately after in the same browser context — "My Workspaces" correctly absent, no stale carryover); test fixtures had been lost to a Postgres container restart between sessions (unrelated to this brief) and were recreated with the same fixed UUIDs/emails before re-verification. Full detail in `BUILT.md`'s `eami-ui` section and `BACKLOG.md`'s B-216 entry.

## Active decision thread (2026-09-23, older) — B-215 built: real Workspace-mode UI, built only on real data. Task brief's own title called this "B-210" — `BACKLOG.md` already has an unrelated B-210 (`PolicyCreateRequest`/`PolicyUpdateRequest.Status` allow-list validation, a prior session's entry) — a genuine numbering collision, checked and flagged rather than silently overwritten; used B-215 instead (the real next free counter value). Part A investigation, reported before any code: confirmed real/buildable now — `GET /v1/workspaces/mine` (the one real membership signal, since `GetWorkspace`/`ListWorkspaces` are NOT membership-gated — a real fail-open trap avoided by checking), `/{id}/members`, and the full `/{id}/policies` CRUD (B-209, built, never had a UI). Confirmed with direct in-schema evidence (migration 000021's own comment) that AI Usage Health/Monthly Spend/Team Adoption have zero real backend support — `audit_log`/`token_usage` have no `workspace_id`, Spend's "$600 monthly allocation" has no backing budget concept anywhere, Team Adoption has no human-user-to-AI-tool-usage data model at all — none built, per the brief's own explicit instruction not to fabricate. Found workspace "description" was real, unexposed data (`groups.description`, never joined) rather than something to invent — one small backend join added. Shipped a deliberately separate `WorkspaceShell` (not nested under Admin's `AppShell`, DESIGN_SYSTEM.md §0's "don't blend modes"), Overview + Our Policies pages, with `PoliciesPage.tsx`'s badges/form extracted into shared components so both views use one real implementation. Mandatory code review found 4 real issues (swallowed query errors, a `useToast()` standing-rule violation, a missing delete error handler, duplicate route blocks), all fixed. **Live verification itself then found 2 more real bugs neither review pass caught:** editing any workspace-scoped policy with conditions 500'd — `UpsertPolicyCondition`'s `ON CONFLICT (policy_id)` had no matching UNIQUE constraint, a pre-existing bug equally affecting the org-wide policy-edit path too, not introduced by this brief — fixed via migration 000023 after confirming zero duplicate rows; and `ListWorkspacePolicies` never joined the workspace's real name, so its own `ScopeBadge` wrongly showed "Global floor" for workspace-scoped policies — fixed with the same join B-214 established. Live-verified with a real Playwright browser against all 4 acceptance criteria in one script against real seeded data — all pass. Full detail in `BUILT.md`'s `eami-api`/`eami-ui` sections and `BACKLOG.md`'s new B-215 entry.
...
components/state: PoliciesPage.tsx/ToolsPage.tsx/WorkflowsPage.tsx/
SettingsPage.tsx (API Keys, Model Pricing tabs) each threaded their own
already-existing create handler into the already-existing prop, the exact
mechanism AlertsPage.tsx's Alert Rules tab already proved works.
AgentsPage.tsx's bare <p>No agents registered yet.</p> (no EmptyState
import, no icon, no button) replaced with a real EmptyState + action,
matching its peers. One additional real match found during
implementation: SettingsPage.tsx's Users tab used DataTable's plain
emptyMessage string, not EmptyState at all -- converted and wired the
same way.

Confirmed, not assumed, correctly left description-only: AuditPage.tsx/
DiscoverPage.tsx/FinOpsPage.tsx/ApprovalsPage.tsx/AssetsPage.tsx (grepped
each, zero create-action handlers) and SettingsPage.tsx's License tab
(its real next step, the upload form, is already inline in the same tab).

Live-verified with a real Playwright browser against a fresh, genuinely
empty throwaway org: 5 of 7 fixes fully click-through verified (empty
state renders -> button click -> exact real create panel opens). 2 of 7
(Model Pricing, Users) are structurally unreachable in an empty state on
this shared dev stack (model pricing is global seed data, never actually
zero; a Users-tab empty state is logically impossible for a real
logged-in session to ever see) -- verified instead by handler-equivalence
against each page's own already-working header button, disclosed as the
weaker form of verification rather than silently claimed as full live
click-through. No-regression check confirmed all 7 touched pages' real
populated views still render correctly and all 5 untouched pages still
load cleanly. tsc/vite build clean, zero console/page errors, all
fixtures removed.

Full detail in BUILT.md's eami-ui section and BACKLOG.md's new B-219
entry. Previous entries, preserved below:

Also this session (2026-09-24/2026-09-23), not individually re-summarized
here -- see the Active decision thread log above for full detail on each:
MATURITY_AUDIT.md written and committed (Horizon 0 usability/maturity
audit, investigation only, no code -- full Parts A-D for all 11 pages, a
priority-order recommendation, explicitly written to a durable file per
direct instruction rather than left only in chat context);
rheoARC_Roadmap_Enterprise_AI_Platform.md committed to the repo root,
closing the roadmap-read rule's own enforcement gap (the file didn't
exist anywhere on this machine when the rule was first added, flagged
rather than fabricated, then supplied by the founder and committed); the
roadmap-discipline standing rule itself added to CLAUDE.md's Conventions
section.

2026-09-23 by Claude Code — B-217 DONE: B-196 increment
1, real CMDB asset list (endpoints + gateway_agents + gateway_tools only,
no fabricated relationships) -- the first real code shipped for the CMDB
epic. Part A/B/C investigation (reported before any code) confirmed real
data exists for endpoints/gateway_agents/gateway_tools; confirmed the AI
Workload CI category (Deployed LLM Instance, Training Job) has zero real
backend anywhere (B-151/B-147 both still zero-build), stays design-only;
confirmed gateway_tools/gateway_nodes have no workspace_id column at all;
confirmed the only real multi-hop relationship today is B-200's own
GetAgentConnections; confirmed zero real admin-configurable CI-
classification backend exists. Corrected a real error in BACKLOG.md's
B-196 extension note in place (models/cloud_clients aren't real table
names -- the real tables are endpoint_ai_apps/endpoint_mcp_servers/
endpoint_model_files).

Part A decision, reported before building: gateway_tools rows get a
visually distinct "Not workspace-scoped" badge (dashed border, muted
gray), never the same "Global floor" gray ScopeBadge uses for a real-but-
null workspace assignment on endpoints/agents -- conflating the two would
falsely imply a scoping mechanism gateway_tools doesn't have
(DESIGN_SYSTEM.md §7.4).

Shipped: GET /v1/gateway/agents/{agentId} and GET /v1/endpoints/
{endpointId} now merge real workspace_id/workspace_name (B-214's merge-
not-extend pattern). New pages/cmdb/AssetsPage.tsx merges the 3 already-
existing list endpoints into one categorized table with static taxonomy
tags (not the admin-configurable panel DESIGN_SYSTEM.md §7.2 describes,
explicitly deferred) and workspace badges. Endpoint rows reuse
DiscoverPage.tsx's existing EndpointDrawer directly; agent rows open a new
lightweight panel reusing B-200's useAgentConnections hook as a plain list
(deliberately not the relationship-graph canvas); tool rows open a new
read-only panel with no relationship section at all (no real relationship
data exists for tools). New "Assets" nav entry flagged against CLAUDE.md's
one-spine rule in a code comment, judged a legitimate exception.

Mandatory code review found 2 real issues, both fixed: GetAgent/
GetAgentEndpoint originally reused the list-oriented workspace-lookup
helpers for a single row (O(all workspace-scoped rows in org) instead of
O(1), deviating from GetPolicy's own correct B-214 precedent) -- fixed
with dedicated single-ID-scoped helpers; AssetsPage.tsx silently capped
endpoints at 200 rows with zero UI indication -- fixed with an explicit
truncation banner. Mandatory security review: zero real findings (new raw
SQL confirmed parameterized and org-scoped; one non-blocking design note
about the workspaces JOIN not independently re-checking w.org_id, not
exploitable today since no live write path sets these columns yet).

Live-verified with a real Playwright browser: one real existing endpoint
and one real existing agent assigned to a real pre-existing workspace
directly via psql; /assets rendered exactly 18 rows, cross-checked
directly against psql's own real count for that org (5 endpoints + 9
agents + 4 tools), with exactly 2 real workspace-name badges, exactly 12
"Global floor," exactly 4 "Not workspace-scoped." Zero console/page
errors. New B-218 logged (not fixed): the gateway_tools/gateway_nodes
workspace_id schema gap itself.

Full detail in BUILT.md's eami-api section and BACKLOG.md's new B-217/
B-218 entries. Previous entry, preserved below:

2026-09-23 by Claude Code — B-216 DONE: the last two
honest gaps in the Workspaces epic closed. Part 1: LoginPage.tsx now
routes a pure workspace member (role=viewer + a real membership) directly
to /workspace after login instead of the unusable /dashboard default;
Sidebar.tsx gains a real "My Workspaces" nav entry (gated on a live
GET /v1/workspaces/mine check) for two-hat users, who previously had zero
visible entry point from Admin's own navigation. Part 2: verified, not
fixed -- WorkspacePoliciesPage.tsx's isAdmin already resolves from the
real per-workspace role and already gates every mutation affordance; no
gap existed, no code changed, only live verification to prove the claim.

Mandatory code review found 4 real issues, all fixed: (1) authStore.ts's
logout() never cleared the shared react-query cache -- a second user
logging in within the same SPA session (no full reload) could momentarily
see the first user's cached data, e.g. a stale "My Workspaces" entry --
fixed by adding queryClient.clear() to logout(), closing this for every
current/future query hook at once. (2) the new nav entry's tension with
CLAUDE.md's one-spine rule -- disclosed and reasoned through explicitly in
a Navigation.tsx code comment rather than silently resolved: it is a
navigation entry point into an already-separate, already-reviewed mode
(B-215), introducing no new identity/audit/policy mechanism of its own.
(3) Sidebar.tsx hand-duplicated the "My Workspaces" NavLink markup in both
the collapsed and expanded render branches -- fixed by adding an optional
`conditional?: 'hasWorkspaces'` field to NavItem and folding the entry
into the existing generic NAV_ITEMS/NAV_GROUPS render loops, eliminating
both hand-copied blocks. (4) LoginPage.tsx's viewer-routing check used a
raw apiFetch for GET /v1/workspaces/mine, bypassing react-query's cache,
so WorkspaceShell's own useMyWorkspaces() immediately re-fetched the same
request on the very next render -- fixed by priming the cache via
queryClient.setQueryData(['workspaces', 'mine'], mine) right after the
fetch.

tsc --noEmit and vite build both clean after all 4 fixes. All 4 Part-1
ACs and both Part-2 cases re-verified live with a real Playwright browser
after the fixes, including a dedicated logout cache-clear check (user A
with a real membership logs out via the real UserMenu button, user B with
zero memberships logs in immediately after in the same browser context --
"My Workspaces" correctly absent, no stale carryover). Test fixtures had
been lost to a Postgres container restart between sessions, unrelated to
this brief's own code -- recreated with the same fixed UUIDs/emails
before re-verification, confirmed live against the real dev server.

Full detail in BUILT.md's eami-ui section and BACKLOG.md's B-216 entry.
Previous entry, preserved below:

2026-09-23 by Claude Code — B-215 DONE: real
...
not exist in eami-api's router at all, both ends swallow the resulting
404 as "not registered yet," and no deployed agent has ever actually
received a remote config update. Elevated to B-129's own severity class
(a real, already-built, currently-100%-non-functional mechanism with zero
visible failure signal) and given its own B-ID rather than left as a
B-164 sub-note. (2) Recommended to be built together with B-164 Part B's
identity-fragmentation fix as ONE bundled brief, not sequenced
separately -- both small, both foundational to Discovery's actual
reliability, likely touching adjacent identity/config code. See Active
decision thread entry above; full detail in BACKLOG.md's new B-165 entry
and B-164's updated Part B note.

2026-09-04 by Claude Code — B-164 investigation complete, no code changes:
Discovery (Module 1) real-state audit. Part A: 12 detection packages exist,
only 10 wired into eami-agent's report; scheduled_tasks (fully implemented)
and file_changes (stub) are orphaned; only 3 of 10 wired domains are
normalized server-side, ai_processes/browser_extensions collected but
never rendered anywhere. Part B: re-confirmed the identity-fragmentation
gap with current code references (endpoints.agent_id is free text, zero
relationship to gateway_agents.id UUID) and found a second, previously
undisclosed manifestation of the same root cause -- eami-agent's
remote-config poll calls a URL that doesn't exist in eami-api's router at
all. Smallest fix estimated as a single-session brief (nullable FK +
manual admin link control), not an epic. Part C: B-139 (agentless
discovery) reconfirmed genuinely zero-built. Part D: Nodes page
reconfirmed exactly as B-080 left it, unrelated to shadow-AI Discovery.
Part E: recommend closing the identity-fragmentation gap first, before
expanding detection breadth or starting B-139. See Active decision thread
entry above; full detail in BACKLOG.md's new B-164 entry.

2026-09-04 by Claude Code — B-163 DONE: Approvals reclassified out of the
sidebar's "Operations" (analysis) group into its own distinct "Approvals"
group, between Gateway and Operations. Small, cosmetic, no functional
change -- ApprovalsPage.tsx untouched. Flagged before building: the task
brief's referenced "Navigation and Journey Architecture findings" don't
actually exist anywhere in this file or BACKLOG.md (grepped directly);
the real misclassification was confirmed instead by reading
Navigation.tsx directly. Live-verified via the real dev server's
live-served module content (browser screenshot unavailable this session,
disclosed). See Active decision thread entry above; full detail in
BUILT.md and BACKLOG.md's new B-163 entry.

2026-09-03 by Claude Code — B-119/B-120/B-142 DONE: token-issuance
rate-limit hardening + a stale doc-comment fix. B-142: registry.go's
LookupByNameAndOrg/LookupByName doc comments corrected to the real,
current org-scoping design (re-verified via fresh grep first). B-119:
POST /v1/gateway/tokens's per-agent rate limit is now env-configurable
(config.RateLimitConfig.TokenIssuePerAgent/WindowSeconds), mirroring
B-070's WorkflowRunPerAgent pattern. B-120: the approved plan's own
per-remote-IP request-rate design was NOT what shipped -- this session's
mandatory security review found it would collapse into one shared bucket
across every org behind the real production Caddy-fronted topology
(B-071), letting a single costless flood lock out all legitimate token
issuance, or even ordinary legitimate multi-agent load self-trigger it.
Redesigned as a bounded, global, unkeyed concurrency gate
(TOKEN_ISSUE_PREAUTH_MAX_CONCURRENT, default 10) with no fixed window to
exploit. A second code-review finding (constructor-argument transposition
risk) fixed via a new named IssueRateLimits struct. Live-verified against
the real rebuilt gateway container, including a LOCK TABLE-based technique
to force genuine DB-call concurrency (plain curl bursts against a fast
local Postgres could not reliably trigger it). New discovered work,
disclosed not fixed: B-161 (a RecordIssued fire-and-forget test race,
same mechanism as the already-diagnosed B-127) and B-162 (B-070's and this
session's rate-limit env vars aren't actually wired through
docker-compose.yml/docker-compose.prod.yml). See Active decision thread
entry above; full detail in BUILT.md and each B-ID's own BACKLOG.md entry.

2026-09-03 by Claude Code — B-122/B-140/B-123/B-126/B-127 test-infrastructure
hardening batch, all four DONE, test-only, zero production code touched.
New shared eami-gateway/internal/testdb package (NewThrowawayPool) mirrors
bootstrap_test.go's per-test throwaway-database pattern, adopted across all
11 real-Postgres pool-creation call sites in 9 eami-gateway packages --
closes B-122's orphaned-audit_log-row accumulation and B-140's confirmed
hash-chain-continuity race in one motion. B-123: deleted the now-redundant
allowAllRulesRealPolicyID() duplicate. B-126: testenv_test.go's dispatch
closure no longer discards audit-write errors. B-127: new waitForEpisodeRow
poll closes the recordEpisodeHook detached-goroutine test race without
touching production code. Verified: go build/vet clean, full
go test ./... -count=1 green 5 times in a row, the specific test that
motivated B-140 additionally run 20 times in isolation (20/20 pass), zero
leaked throwaway databases. Full detail in BUILT.md and each B-ID's own
BACKLOG.md entry.

2026-09-03 by Claude Code — B-160 logged, EPIC, investigation not
started: Enterprise AI Operating System, persona-based workflows.
Flagged as the largest, most transformative epic identified this entire
session, recommending its own dedicated strategic session rather than
casual/piecemeal building. Core insight: existing Identity->Action->
Tool->Policy->Cost/Audit pipeline needs new workflows/views per persona
(Compliance/Legal, Finance, Data Science/ML, business leaders, individual
employees, executives, Procurement, HR/People), not new underlying data
or a parallel system. Compliance/Legal regulatory mapping and reporting
flagged as the single most concrete, valuable piece and recommended as
the epic's first standalone investigation. Connects to and elevates
B-157 (RBAC x Licensing as the workflow-selection mechanism), B-078
(vendor management, extended by the Procurement persona), B-130
(confirmed employee-facing home), B-147/B-151 (existing Data Science/ML
scope, referenced not duplicated). B-ID confirmed free before minting.
No code changes. See Active decision thread entry above; full detail in
BACKLOG.md's new B-160 entry. Counter now stands at B-161.

2026-09-03 by Claude Code — B-158/B-159 logged, not started: two features
sourced from direct, hands-on UX research against TrueFoundry's actual
live trial instance (not marketing material). B-158 (Medium): curated
one-click connector/MCP registry (GitHub, Slack, Salesforce, Atlassian,
etc.), reducing setup friction versus today's manual REST-API-config
(B-044/046) or OpenAPI-spec-paste-only (B-075) paths. B-159 (Low, smaller):
hosted STDIO-to-HTTP MCP server wrapping, letting a customer host a
local/stdio MCP server as a real network-accessible streamable-HTTP
server — no current EAMI equivalent. Separately, the same research
confirmed EAMI's existing MCP architecture (B-061 tools/list, B-075
OpenAPI auto-discovery) matches 2 of TrueFoundry's 4 core "advanced" MCP
connection paths — real external validation the architecture decisions
were sound. Both B-IDs confirmed free before minting. No code changes.
See Active decision thread entry above; full detail in BACKLOG.md's new
B-158/B-159 entries. Counter now stands at B-160.

2026-09-02 by Claude Code — B-157 logged, EPIC, investigation not started:
modular licensing & entitlement system for the three-module product
structure (Discovery/Gateway/AI Infrastructure). Core design challenge
flagged as needing deliberate resolution first: on-prem/air-gapped
(ADR-020) rules out a live billing-API check, so a signed local license
file is the real candidate, with a genuine offline-only-vs-check-in
tradeoff not yet decided. Eight required components logged (data model,
dispatch-layer enforcement, RBAC x Licensing connecting to B-113,
visibility/UX as a founder go-to-market call, admin UI reusing Settings,
vendor issuance, trial/expiry behavior, audited license-change events
per B-121/124/125) plus three carried-forward requirements (cross-module
lineage, an identity-fragmentation priority escalation honestly flagged
as having no logged B-ID of its own, and a UI-consistency mandate reusing
DataTable/B-104 and deep-linking/B-092). B-ID confirmed free before
minting. No code changes. See Active decision thread entry above; full
detail in BACKLOG.md's new B-157 entry. Counter now stands at B-158.

2026-09-02 by Claude Code — B-156 investigation complete, no build brief
started: pattern-based sensitive-data redaction on the ai_provider
dispatch path, flagged as the session's most repeated unbuilt gap. Traced
the real chokepoint (aiprovider.Router.Dispatch, two independent call
sites into it) and confirmed B-102's DispatchHook mechanism does NOT fit
(post-dispatch, observational-only) -- a new small interception step is
needed instead, not a hook. Recommended replicating Presidio's proven
regex+checksum pattern-design natively in Go (zero new dependency, verified
via live research), MASK over tokenize for v1, a non-hashed audit count
field matching DataHandling's (B-078) precedent, per-connector config
matching data_handling_designation/action_paths' existing patterns, and
AES-256-GCM (toolcreds precedent) for tokenization if picked up later.
Honest estimate: a real multi-brief epic. No B-ID for a build brief
minted. See Active decision thread entry above; full detail in
BACKLOG.md's new B-156 entry. Counter now stands at B-157.

2026-09-02 by Claude Code — B-155 logged, EPIC, investigation not started:
conditional workflow branching (structural via B-063/064's extraction
mechanism, or LLM-based via a governed ai_provider dispatch). Flags the
original "branching architecturally blocked" finding (B-058 onward) as
predating the extraction mechanism and needing real re-examination, not
inheritance. Real open questions on audit (B-124/125), TOCTOU (B-057),
and SwitchStep's UI-only status (B-131) once backend support might
exist. This item was discussed earlier the same session but never
actually logged -- the user's "just logged" framing was checked against
the repo, found inaccurate, flagged rather than confirmed, then
corrected for real per the user's own follow-up. See Active decision
thread entry above; full detail in BACKLOG.md's new B-155 entry. Counter
now stands at B-156.

2026-09-02 by Claude Code — B-150/B-151/B-152/B-153/B-154 logged, zero
investigation started: five items from a "TrueFoundry feature-parity
initiative" discussion, K8s-native explicitly excluded by deliberate
decision (EAMI's on-prem/SaaS-hybrid single-instance architecture is an
intentional bet, not a gap). Strategic framing locked in: "state of the
art" means the deepest, most adversarially-proven version of each
capability, not feature-checklist parity -- this session's own
security-testing discipline is the real differentiator. B-150 Guardrails
(content safety/PII/prompt-injection, distinct from redaction, fits
B-102's dispatch-hook mechanism), B-151 Model hosting/serving (connects
to B-147/B-130, one more ai_provider Adapter entry), B-152 unified
multi-provider API surface (gated on 2-3 more adapters existing first),
B-153 real performance benchmarking (required before any parity/gap
claim), B-154 app/policy/workflow templates (low priority). B-ID range
confirmed free before minting. No code changes. See Active decision
thread entry above; full detail in BACKLOG.md. Counter now stands at
B-155.

2026-09-02 by Claude Code — B-148 CLOSED, DONE and live-verified per the
user's own close-out: core canvas interactivity (add/remove/reorder/
click-to-configure, zero backend writes) confirmed working live after
the object-identity loop fix (commit 767811d) -- flickering at rest gone,
toolbox-add correctly attaches. Reorder not separately click-confirmed
(shares the identical fixed code path). The original toolbox-drop-
during-DOM-swap theory is now strongly corroborated (predict-then-
observe), not merely inferred -- recorded explicitly, not silently
dropped. New follow-up B-149 logged, parked, not urgent: general UX
refinement of the canvas interactions, cross-referenced from B-131. See
Active decision thread entry above; full detail in BUILT.md's eami-ui
section and BACKLOG.md's B-148/B-149 entries. Counter now stands at
B-150.

2026-09-02 by Claude Code — B-148 addendum: a real infinite destroy/
recreate loop in the canvas, found by the user's own live testing
(continuous flicker, broken toolbox drag-add), diagnosed by reading the
actual library source (not guessed) -- sequential-workflow-designer-react
destroys/rebuilds by reference identity and echoes its own definition
back via onReady after every Designer.create(), and commit() was
unconditionally producing a new object graph on every call, including
that echo, causing a self-sustaining loop from mount. Fixed with a real
content comparison (JSON.stringify) before calling setDefinition.
Verified via a targeted Node test bundling the real, unmodified adapter
code and replicating commit()'s literal guard expression, 7/7 checks
passed (echo loop converges in 1 iteration; real edits still detected;
toolbox-insert survives the round-trip). Real tsc/vite build clean;
served module confirmed over HTTP. Still open: the user's own re-test of
flickering-at-rest/toolbox-add/reorder -- this test proves the round-trip
is idempotent, not a substitute for that. See Active decision thread
entry above; full detail in BUILT.md's eami-ui section (B-148 addendum)
and BACKLOG.md's B-148 entry. Counter unchanged at B-149.

2026-09-02 by Claude Code — B-148 code/build/grep/review-verified DONE:
Workflow Canvas rebuild Brief 2, local interactivity (add/remove/reorder,
click-to-configure via reused StepConfigPanel), zero backend writes.
Library-native toolbox/context-menu/keyboard/selection interactions used
directly, confirmed against the library's real .d.ts before writing code.
Two small pre-approved WorkflowsPage.tsx exceptions (6 export keywords,
1 tooltip string). Mandatory code-review pass found and fixed 2 real
state-management bugs (stale-tools-list label baking; realStepIds
recomputed on background refetch instead of captured once); mandatory
security review found zero HIGH/MEDIUM. AC6 (live click-through)
explicitly NOT done this session -- claude-in-chrome unavailable,
disclosed as open. See Active decision thread entry above; full detail
in BUILT.md's eami-ui section and BACKLOG.md's B-148 entry. Counter now
stands at B-149.

2026-09-02 by Claude Code — B-146 DONE: session persistence on hard reload
(localStorage via Zustand persist, chosen because the HttpOnly-cookie
option needed backend changes this fix's scope explicitly forbade) and
redirect-back-after-login (a module-load-time window.location capture in
authStore.ts, since AppShell.tsx/router.tsx were out of scope). A planned
pre-emptive refresh-token expiry check was removed after live
verification found eami-api's refresh_token is opaque, not a JWT --
would have forced a logout on every reload otherwise. Live-verified via
the real backend (real login/refresh calls) and the actual shipped
authStore.ts executed via an esbuild bundle under simulated browser
globals, 11/11 assertions passing; no browser click-through (declined
again this session), disclosed. See Active decision thread entry above;
full detail in BUILT.md's eami-ui section and BACKLOG.md's B-146 entry.
Counter unchanged at B-148.

2026-09-02 by Claude Code — RAGFlow logged as a candidate RAG framework
under B-130's RAG addendum, with license research: default backend
Elasticsearch (triple-licensed AGPLv3/SSPL/Elastic License 2.0) is
low-risk to embed unmodified as an internal backend, but recommended
against anyway in favor of Apache 2.0 Infinity (RAGFlow's own database)
or OpenSearch, for a compliance-focused product. One canonical entry
under B-130, cross-referenced (not duplicated) from B-147's dataset-
management item. No code changes. See Active decision thread entry
above; full detail in BACKLOG.md's B-130 entry. Counter unchanged at
B-148.

2026-09-02 by Claude Code — B-147 license reference section added, no code
changes: Axolotl/Ludwig (Apache 2.0, patent grant + retaliation clause),
Gitleaks core engine (MIT, distinct from the separately-licensed
gitleaks-action CI wrapper; Betterleaks successor's license unchecked).
Practical THIRD_PARTY_LICENSES obligation logged, same pattern as
sequential-workflow-designer's MIT license. Research only, not legal
advice. See Active decision thread entry above; full detail in
BACKLOG.md's B-147 entry. Counter unchanged at B-148.

2026-09-02 by Claude Code — B-147 logged, EPIC, investigation not started:
customer-controlled LLM training orchestration, an orchestration layer
over customer-run Axolotl (Apache 2.0), not an EAMI-built training
engine. Six components identified (guided config UI, dataset management
tied to the RAG capability, GPU resource visibility, experiment
tracking, model registry/deployment via the existing ai_provider Adapter
pattern, vendor-hosted fine-tuning support). Scale comparable to B-130,
its own epic. B-ID confirmed free (B-146 already taken, counter stood at
B-147) before minting. No code changes. See Active decision thread entry
above; full detail in BACKLOG.md's B-147 entry. Counter now stands at
B-148.

2026-09-02 by Claude Code — B-130 scope addendum, no code changes: dynamic
agentic loop confirmed in-epic (two paths, (a) gateway-orchestrated not
requiring B-076, (b) remote-MCP requiring B-076 + an unverified external
Anthropic dependency; new open design question on mid-loop escalation
blocking), RAG added in-epic (one more governed tool call, but a real
embedding/vector-store pipeline doesn't exist yet), LLM training/
fine-tuning split into its own separate future epic (three distinct
possible meanings flagged, not yet clarified). See Active decision thread
entry above; full detail in BACKLOG.md's B-130 entry. Counter unchanged at
B-146.

2026-09-01 by Claude Code — B-145 closed out for real: a genuine blank-
canvas bug (found only by actual human visual verification, not by any
build/type-check/code-review pass) traced to sequential-workflow-
designer's unstyled mount div needing a container with definite size in
BOTH axes -- fixed in two passes (h-[75vh] for height, flex->grid for
width, each independently confirmed via the user's own DevTools
measurements). A separate uncaught browser error (reportAllChanges/
startTime) traced and confirmed via grep to be entirely absent from this
project's dependency tree -- an unrelated browser extension, correctly
left unfixed. A differential two-workflow test confirmed the adapter
itself never had a bug. Temporary DashboardPage.tsx verification button
fully reverted (zero diff vs. committed state); replaced by one small,
permanent, explicitly-requested Preview link on WorkflowsPage.tsx's
existing row actions -- purely additive, zero change to any existing
edit/delete/reorder/save logic. See Active decision thread entry above.

2026-09-01 by Claude Code — B-146 logged (not fixed): auth session
doesn't survive a hard page reload/direct URL nav (in-memory-only
authStore, no persistence), and login never redirects back to the
originally-requested page (always /dashboard). Found live while manually
verifying B-145 -- real, pre-existing, app-wide gap, not specific to the
new canvas-preview route or caused by B-145. Also added a TEMPORARY
button on DashboardPage.tsx (clearly marked, to be removed after manual
verification) so the canvas-preview route can be reached via real
client-side navigation instead of a session-losing URL paste. See Active
decision thread entry above. Counter now stands at B-147.

2026-09-01 by Claude Code — B-145 DONE: Workflow Canvas rebuild Brief 1
(sequential-workflow-designer, read-only). New route at
/gateway/workflows/:id/canvas-preview, zero edits to WorkflowsPage.tsx
(the card editor), sequential-workflow-machine explicitly not adopted.
Both premises B-131's investigation flagged as needing empirical
confirmation (free-tier custom-step-type support, Tailwind/Vite
compatibility) resolved by actually building and testing, not by
re-reading either conflicting doc source -- both confirmed working.
Every interactive surface disabled individually, zero mutation hooks
anywhere in the new files (confirmed by grep), real measured bundle
delta (+29.3 kB gzip JS). Real tsc/vite build clean; eami-ui's dev
container rebuilt and the new page confirmed serving error-free through
the real dev server; WorkflowsPage.tsx re-confirmed unaffected.
claude-in-chrome was offered and declined this session -- actual
browser-rendered DOM was not visually confirmed, disclosed plainly. See
Active decision thread entry above. Counter now stands at B-146.

2026-09-01 by Claude Code — B-144 DONE: optional offsite backup transport
(age + rclone, chosen over restic/GPG for fitting the "customer holds
the only key" requirement and this codebase's small-direct-tools
discipline). New scripts/backup-offsite.sh/restore-offsite.sh +
docker/eami-backup/Dockerfile, gated structurally (no separate flag) on
OFFSITE_AGE_RECIPIENT/OFFSITE_RCLONE_REMOTE/a real secrets/rclone.conf
all being present. Encryption happens before any network access; EAMI's
own process never holds a decryption key. AC5 (whether whole-file
encryption is enough for unencrypted JSONB/audit content) resolved
explicitly and confirmed empirically, not left ambiguous. Live-verified
against a real MinIO destination and a real age keypair: all 5
acceptance criteria confirmed, including a full offsite restore drill
from a second, fully separate isolated stack (simulating total original-
host loss) that recovered via restore-offsite.sh into the unmodified
B-143 restore-db.sh and passed the complete B-134/B-143 regression
checklist with zero regressions. See Active decision thread entry above.

2026-09-01 by Claude Code — B-143 DONE: new scripts/restore-db.sh fixes
the hypertable FK constraint loss B-134's restore drill found (pg_restore
always emits ALTER TABLE ONLY, which TimescaleDB hypertables reject on
ADD CONSTRAINT the same way B-029 already found for --clean's DROP
CONSTRAINT). Fix is generic (parses pg_restore's stderr for the
hypertable-error/ONLY-ADD-CONSTRAINT shape, not hardcoded to today's 2
tables) and fails loudly on any other kind of restore error rather than
guessing -- verified against a synthetic mixed-error fixture. RECOVERY.md
corrected (no initdb.d auto-apply since B-051; pg_restore recreates
schema from the dump regardless). Live-verified via a real isolated
restore drill matching B-134's own non-destructive methodology: all 4
constraints repaired, pg_constraint byte-identical to live source, full
B-134 regression checklist (hash chain, workflow/approval state,
credential decryption) re-run clean. See Active decision thread entry
above.

2026-09-01 by Claude Code — B-134 investigation complete (no code changes):
real fresh restore-test drill against current schema (isolated stack,
zero impact to real dev stack, torn down after) found the backup
mechanism's coverage is sound (full-database pg_dump, zero maintenance
burden as tables are added) and the hash chain survives restore with 0
bad hashes across 1636 rows (independently reverified). Found and logged
fresh: B-143 (High -- pg_restore silently drops 4 hypertable FK
constraints on gateway_node_metrics/paste_events, fix confirmed by hand)
and B-144 (EPIC -- offsite backup mechanism design, grounded in ADR-020
Model A, recommendation only, not built). See Active decision thread
entry above. Counter now stands at B-145.

2026-09-01 by Claude Code — B-141 DONE: JWT org-binding (identity.Claims/
IssueRequest gained OrgID, additive, HandleIssue sets it from the
already org-scoped key.OrgID) plus all 4 confirmed unscoped
registry.LookupByName call sites (mcp ServeSSE, workflow HandleRun,
workflow RateLimitRunMiddleware, episode authenticateCaller) switched to
the pre-existing LookupByNameAndOrg -- closes the confirmed cross-tenant
episode-content read this session's B-128 security review found. Hard
cutover at 3 of 4 sites (pre-fix tokens rejected 401); RateLimitRun-
Middleware's best-effort pass-through confirmed safe by both mandatory
review passes (HandleRun's own independent check is the real boundary).
Both review passes clean; the one shared finding (stale registry.go doc
comment, out of this brief's scope) logged fresh as B-142. Three new
real-Postgres tests prove org isolation across all 3 consequential
surfaces, matching B-128's own two-org standard -- the episode test's
negative control was manually confirmed to actually catch the bug (fix
reverted, test failed with the exact predicted leak, fix restored, test
passed again), not merely asserted. Full go test ./... green across
eami-gateway, zero regressions to B-128/B-098/B-042. Live-verified
against real Postgres and the rebuilt/restarted gateway container. See
Active decision thread entry above. Counter now stands at B-143.

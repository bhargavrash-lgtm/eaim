...
- **B-129 (2026-08-28): CRITICAL, logged not fixed — policy hot-reload is completely non-functional on the actual dispatch path; only a full gateway restart makes any policy change take effect, for any org. Per explicit user direction: more urgent than B-128, far more urgent than B-124/125.** Found root-causing why B-128's own investigation still couldn't get a live escalate policy to match even after two confirmed, successful `pg_notify` reloads. Ruled out the mundane stale-process/cache/build explanation first, per explicit user direction: the gateway container/image were freshly built minutes before both failed dispatch attempts, and gateway logs show both reloads (`04:28:07`, `04:30:15`, both `"loaded policies from DB","count":3`) landing before their respective dispatches — `pLoader`'s own internal state genuinely updated both times. Real cause, confirmed by an exhaustive grep (`.Evaluator()` has exactly 2 production call sites in the whole module): `main.go:212` and `:244` both call `pLoader.Evaluator()` once, at startup, and store the frozen result directly in `Dispatcher.policyEval` / `workflowExecutor` — `Dispatch()` (`dispatcher.go:412`) evaluates against that one snapshot forever after. `pLoader.Listen()` correctly updates the loader's own atomic pointer on every reload, but nothing after startup ever calls `.Evaluator()` again to observe it — a direct violation of `internal/policyloader/loader.go`'s own documented usage contract (`loader.Evaluator().Evaluate(...)`, called fresh "in the dispatch closure"). The startup log line `"policy engine ready (DB-backed, live reload enabled)"` is false for the path that actually matters. **Investigation in progress, fix explicitly withheld pending it and separate approval:** (1) correct fix — call `pLoader.Evaluator()` fresh per-dispatch (matching the documented contract) vs. a deliberate performance reason for the snapshot-once pattern needing a different reload-aware mechanism; (2) whether `workflowExecutor`'s separate construction at `main.go:244` has identical or different exposure; (3) whether tonight's prior live-verification claims (B-086, B-090, B-098, B-116, B-118) that touched policy state actually proved what they claimed, given this bug. B-124/125 remain paused; `main.go` not yet touched; no fixtures cleaned up. See B-129's full `BACKLOG.md` entry for evidence.
- **B-128 (2026-08-28): URGENT/Critical, logged not fixed — no `org_id` scoping exists anywhere in the policy evaluation pipeline; every tenant shares one global, unfiltered policy list.** Found mid-live-verification of B-124/125: `eami-gateway/internal/policyloader/loader.go`'s `queryRules()` has no `org_id` predicate and doesn't even SELECT it; `eami-policy/types.go`'s `Rule`/`Conditions`/`ActionContext` have no `OrgID` field at all — there is nothing downstream to filter on even if the loader tried. `Loader.store()` builds exactly one global `policy.Evaluator` shared by the entire process across every org. Confirmed live in the shared dev DB: 3 active policies from 2+ different orgs sitting in the same priority-ordered evaluation list. This means any org's dispatch can be matched, allowed, denied, or escalated by another org's policy today, in production — a real multi-tenant isolation failure in the core enforcement surface, not test noise. **Per explicit user direction: this is now top priority, full stop.** B-124/125 are PAUSED (not marked DONE, live-verification fixtures left in place, not cleaned up) until the founder decides whether B-128 blocks their completion. Second open question, under active investigation per explicit user direction: why a `priority=-100`, empty-conditions policy still failed to match on a second live attempt even after this cross-org finding — ruling out the mundane stale-process/cache/build explanation (same class as B-055/B-062/B-104) before considering anything more complex. See B-128's full `BACKLOG.md` entry for evidence.
...
## Last updated
2026-08-28 by Claude Code — B-129 logged, CRITICAL, not fixed: policy
hot-reload is completely non-functional on the actual dispatch path --
main.go snapshots pLoader.Evaluator() once at startup (2 call sites,
lines 212/244) instead of calling it fresh per dispatch as the
policyloader package's own doc comment specifies, so pg_notify reloads
update the loader's internal state but Dispatch() never observes it;
only a full gateway restart makes any policy change take effect, for
any org. Confirmed via gateway logs (2 successful reloads, both before
their dispatch attempts, both still evaluated against the startup
snapshot) and an exhaustive grep ruling out any other call site. Per
explicit user direction this is more urgent than B-128 and far more
urgent than B-124/125 -- investigation (not fix) is in progress: correct
fix shape, workflowExecutor's exposure, and whether tonight's prior
live-verification claims (B-086, B-090, B-098, B-116, B-118) actually
proved what they claimed given this bug. main.go not touched, B-124/125
still paused, no fixtures cleaned up. See Active decision thread entry
above. Counter now stands at B-130.
